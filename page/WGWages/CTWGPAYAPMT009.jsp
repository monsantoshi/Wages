						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWgRwVinaiService.addListApprove(feeWgRwVinai, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWgRwVinaiService.addListApprove(feeWgRwVinai, false);
					
								}else{
								if( allRowUpdate == myUpdate.length )
									FeeWgRwVinaiService.addListApprove(feeWgRwVinai, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
								else
									FeeWgRwVinaiService.addListApprove(feeWgRwVinai, false);
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYAPMT009] บันทึกรับรองรายการหักกรณีวินัย (ลูกจ้างเหมา)
		</td>
	</tr>
</table>
				    ...
				    if(tab.rows.length>1){
						for(i=0; i<tab.rows.length; i++){
							...
							if (chk[i].checked){
									...
									FeeWgRwVinaiService.deleteRwVinai(rwVinai, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร7');}});
									chDelete = true;
								}
						...
						if(tab.rows.length==1){
							...
							if (chk[0].checked){
									...
									FeeWgRwVinaiService.deleteRwVinai(rwVinai, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร8');}});
									chDelete = true;
								}
