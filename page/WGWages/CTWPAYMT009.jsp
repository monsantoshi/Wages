...
<script type="text/javascript">
	...
	function countData(){
		...
		FeeWpayRwVinaiService.countData
		(
			'<%=userId%>',
			'<%=ouCode%>',
	...
	function whenShowDataTable()
	{
		...
		FeeWpayRwVinaiService.findByCriteria
		(
			'<%=userId%>',
			'<%=ouCode%>',
			parseInt(DWRUtil.getValue("year")),
	    ...
	    if(tab.rows.length>1){
			for(i=0; i<tab.rows.length; i++){
				...
				if (chk[i].checked){
						...
						FeeWpayRwVinaiService.deleteRwVinai(emp, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
					}
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYMT009 ] รายการหักกรณีวินัย ตัดค่าจ้าง และขาดงาน
		</td>
	</tr>
</table>
