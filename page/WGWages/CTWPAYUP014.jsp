...
<script type="text/javascript" src="dwr/interface/FeeWpayRwVinai3Service.js"></script>
...
<!--
	...
	function whenQueryData()
	{
		...
		FeeWpayRwVinai3Service.findByEmpCodeDetail('<%=empID%>','<%=ouCodeEdit%>','<%=yearEdit%>','<%=periodEdit%>',{callback:whenQueryDetailHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		FeeWpayRwVinai3Service.findByEmpCodeList('<%=userId%>','<%=ouCodeEdit%>','<%=yearEdit%>','<%=periodEdit%>','<%=empID%>',{callback:whenQueryListDataHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	...
	function writeSelect(inname,emp,key)
	{
		...
		if(<%=confirm%>){
		return "<div align='center' style='background-color:#CCCCCC;'><select name='"+inname+"' disabled='true' style='background-color:transparent;color:#000000'><option value='N' "+temp1+" >รายการปกติ</option>"+
		...
		return "<div align='center' ><select onchange='addListUpdate("+key+")' name='"+inname+"'><option value='N' "+temp1+" >รายการปกติ</option>"+
												"<option value='A' "+temp2+">ปรับปรุงรายการหัก</option>" +
												"<!--<option value='R' "+temp3+">ปรับปรุงรายการรับเรียกคืน</option>" +
												"<option value='B' "+temp4+">ตกเบิกปรับปรุงรายการรับ</option>" +
												"<option value='S' "+temp5+">ตกเบิกรายการรับเรียกคืน</option>--></select></div>";
	...
	var rwVinai3 = {keySeq:null, ouCode:null, yearPr:null, periodPr:null,empCode:null,confirmFlag:null,
	...
	function onUpdate()
	{
		...
		{  
		 ...
		 if (yearWWork<=yearEdit && periodWWork != null) {
			update = null;
			row = tab.rows[i];
			rwVinai3.keySeq = parseInt(keySeq[i].value);
			
			if(tab.rows.length == 1){
				rwVinai3.flagPr = flagPr.value;
				//rwVinai3.periodWork  = periodWork.value;
			}else{
				rwVinai3.flagPr = flagPr[i].value;
				//rwVinai3.periodWork  = periodWork[i].value;
			}
			rwVinai3.periodWork  = periodWork[i].value;
			//if (yearWork[i].value != ''){
			//	rwVinai3.yearWork  = yearWork[i].value;
			//}
			//else{
			//	rwVinai3.yearWork  = null;
			//}
			rwVinai3.yearWork  = '<%=yearEdit%>';
			if (decDay[i].value !=''){
				//rwVinai3.decDay  = parseInt(decDay[i].value);
				rwVinai3.decDay  = decDay[i].value;
			}
			else{
				rwVinai3.decDay  = null;
			}
			
			if (remark[i].value !=''){
				rwVinai3.remark  = remark[i].value;
			}
			else{
				rwVinai3.remark  = null;
			}
			rwVinai3.updBy = '<%=userId%>';
			...
			if(update){
				...
				if(aRows.length  > num + 1 ){
					if( allRowUpdate == myUpdate.length )
						FeeWpayRwVinai3Service.addList(rwVinai3, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
					else
						FeeWpayRwVinai3Service.addList(rwVinai3, false);
					
				}else{
					if( allRowUpdate == myUpdate.length )
							FeeWpayRwVinai3Service.addList(rwVinai3, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
						else
							FeeWpayRwVinai3Service.addList(rwVinai3, false);
		     ...
		     alert("ใส่ปีไม่ถูกต้อง หรือ  เดือนต้องไม่เกินเดือนปัจจุบัน ระบบยกเลิกการบันทึก");
		     RevertData();
		   }
		}
	...
	function insertNewData()
	{
		...
		DWREngine.beginBatch();
			...
			{
				rwVinai3.keySeq = null;
				rwVinai3.ouCode = '<%=ouCodeEdit%>';
				rwVinai3.yearPr = '<%=yearEdit%>';
				rwVinai3.periodPr = '<%=periodEdit%>';
				rwVinai3.empCode = '<%=empID%>';
				rwVinai3.codeSeq = $("codeSeq").value;
				rwVinai3.yearWork = oRows[i].cells["yearWork"].childNodes[0].value;
				rwVinai3.periodWork = oRows[i].cells["periodWork"].childNodes[0].value;
				rwVinai3.decDay = oRows[i].cells["decDay"].childNodes[0].value;
				rwVinai3.remark = oRows[i].cells["remark"].childNodes[0].value;
				rwVinai3.flagPr= oRows[i].cells["flagPr"].childNodes[0].value;
				rwVinai3.confirmFlag = 'N';
				rwVinai3.updBy = '<%=userId%>';
				rwVinai3.creBy = '<%=userId%>';
				rwVinai3.creDatw = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
				//RwVinai3Service.addList(rwVinai3, {callback:onInsertCallback});
	...
	function checkDay(object){
		...
		if(day != null && day != '' && day != 0){
			...
			switch (month) {
				  case "1":
				    if(day > 31){
				    	alert('จำนวนวันปรับปรุงไม่เกิน 31 วัน');
				    	oRows[rowToCompare].cells["decDay"].childNodes[0].value = '';
				    }
				    break;
				  case "2":
				    if(day > 29){
				    	alert('จำนวนวันปรับปรุงไม่เกิน 29 วัน');
				    	oRows[rowToCompare].cells["decDay"].childNodes[0].value = '';
				    }
				    break;
				  case "3":
				  ...
				  case "4":
				    if(day > 30){
				    	alert('จำนวนวันปรับปรุงไม่เกิน 30 วัน');
				    	oRows[rowToCompare].cells["decDay"].childNodes[0].value = '';
				    }
				    break;
				  case "5":
...
<body>
<form name="formEdit"   action="security.htm?reqCode=CTWPAYMT014" method="post">
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYUP014 ] บันทึกรายการหักได้รับค่าจ้างไม่เต็มเดือน
		</td>
	</tr>
</table>
				...
				<table id="table"  bordercolor="#4A4A4A" align="center" style="text-align: center;width:700px" border="1" cellpadding="0" cellspacing="0" >
					<thead style="text-align: center">
						<tr CLASS="TABLEBULE2" style="height:30px;">
							...
							<th CLASS="TABLEBULE2" style="width:100px" align="center">ประเภทรายการ</th>
							<th CLASS="TABLEBULE2" colspan="2" align="center">ปี/เดือนที่ทำงาน</th>
							<th CLASS="TABLEBULE2" style="width:100px" align="center"><center>จำนวนวันหัก</center></th>
...
<table width="100%" CLASS="TABLEBULE2" >
	...
	<form name="backForm" action="security.htm?reqCode=CTWPAYMT014" method="post">
				    ...
				    if(tab.rows.length>1){
						for(i=0; i<tab.rows.length; i++){
							...
							if (chk[i].checked){
									...
									//alert('BB'+rowDelete);
									rwVinai3.keySeq = parseInt(frm.elements["keySeq"][i].value);
									FeeWpayRwVinai3Service.deleteRwVinai3(rwVinai3, {callback:onDeleteCallback});
									chDelete = true;
								}
						...
						if(tab.rows.length==1){
							...
							if (chk[0].checked){
									...
									//alert('CC'+rowDelete);
									rwVinai3.keySeq = parseInt(frm.elements["keySeq"][0].value);
									FeeWpayRwVinai3Service.deleteRwVinai3(rwVinai3, {callback:onDeleteCallback});
									chDelete = true;
								}
