...
<!--
	...
	function insertNewData()
	{
			...
			if(oRows.length > 2)
			{
				...
				for(var i=oRows.length-1;i >1;i--)
				{
					...
					if( i == 2 )
						FeeWgRwVinai2Service.addList(rwVinai2, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
					else
						FeeWgRwVinai2Service.addList(rwVinai2, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
				}
				DWREngine.endBatch();	
	...
	{
	   	FeeWgPnEmployeeService.findEmpBySecurity('<%=userIdInsert%>','<%=ouCodeInsert%>', {callback:onLoadEmployeeCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	}
    ...
    function onLoadEmployeeCallback(data)
    {
	 ...
	 function whenFetchEmployeeDetail(empCode)
	 {
		...
		FeeWgPnEmployeeService.findByEmpCodeDetailM('<%=userIdInsert%>', empCode, '<%=ouCodeInsert%>', '<%=yearInsert%>', '<%=periodInsert%>', {callback:whenFetchEmployeeDetailCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	 }
...
<body>
<form name="formEdit"  action="security.htm?reqCode=CTWGPAYMT010" method="post">
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYIN010 ] บันทึกรายการหักโดยได้รับค่าจ้างไม่เต็มเดือน จ้างเหมา
		</td>
	</tr>
</table>
				...
				<table width="750" border="1" cellpadding="1" bordercolor="#6699CC" cellspacing="1"  align="center">
        			...
        			<tr>
         				<td class="font-field"  align="right" width="100">เลขประจำตัว</td>
          				<td align="left" width="15"><input type="text" name="empCbo" maxlength="13" style="width:150" onchange="whenSelectEmpOption();"></td>
				...
				<table id="table"  bordercolor="#4A4A4A" align="center" style="text-align: center;width:750px" border="1" cellpadding="0" cellspacing="0" >
					<thead style="text-align: center">
						<tr CLASS="TABLEBULE2" style="height:30px;">
							...
							<th CLASS="TABLEBULE2" style="width:100px" align="center"><center>จำนวนวันที่ถูกหัก</center></th>
						...
						<tr id="temprow" style="visibility:hidden;position:absolute">
							...
							<td id="yearWork"><input type="text" value="<%=yearInsert%>"  name="yearWork" size="6" maxlength="4" align="center" style="text-align:center;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
							<td id="periodWork" align="center" ><select name="periodWork" style="width:150;" onchange="bypass(this);">
...
<table width="100%" CLASS="TABLEBULE2" >
	...
	<form name="backForm" action="security.htm?reqCode=CTWGPAYMT010" method="post">
