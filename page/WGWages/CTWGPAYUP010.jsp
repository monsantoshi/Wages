...
<!--
	...
	function whenQueryData()
	{
		...
		FeeWgRwVinai2Service.findByEmpCodeDetail('<%=empID%>','<%=ouCodeEdit%>','<%=yearEdit%>','<%=periodEdit%>',{callback:whenQueryDetailHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		FeeWgRwVinai2Service.findByEmpCodeList('<%=userId%>','<%=ouCodeEdit%>','<%=yearEdit%>','<%=periodEdit%>','<%=empID%>',{callback:whenQueryListDataHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
			...
			if(update){
				...
				if(aRows.length  > num + 1 ){
					if( allRowUpdate == myUpdate.length )
						...
						FeeWgRwVinai2Service.addList(rwVinai2, false);
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYUP010 ] บันทึกรายการหักโดยได้รับค่าจ้างไม่เต็มเดือน (จ้างเหมา)
		</td>
	</tr>
</table>
          ...
          <td  align="center">
				<table width="750" border="1" cellpadding="1" bordercolor="#6699CC" cellspacing="1"  align="center">
        			...
        			<tr>
        				...
        				<td class="font-field"  align="right" width="65">ตำแหน่ง</td>
				    ...
				    if(tab.rows.length>1){
						for(i=0; i<tab.rows.length; i++){
							...
							if (chk[i].checked){
									...
									FeeWgRwVinai2Service.deleteRwVinai2(rwVinai2, {callback:onDeleteCallback});
									chDelete = true;
								}
