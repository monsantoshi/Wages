...
<script type="text/javascript">
   ...
   /* function whenFetchEmployeeToCallback(data)
	...
	function whenGenerate(){
		...
		FeeWpayRwIncDecOtherService.generate
			(
				'<%=userId%>',
				'<%=ouCode%>',
	...
	{	
		...
		if(document.forms['searchForm'].elements['incDecCbo'].value != null && document.forms['searchForm'].elements['incDecCbo'].value != ''){
			...
			FeeWpayRwIncDecOtherService.findByCriteriaList
			(
				'<%=userId%>',
				'<%=ouCode%>',
				parseInt(DWRUtil.getValue("year")),
	...
	function countData()
		...
		//if(document.forms['searchForm'].elements['incDecCbo'].value != null && document.forms['searchForm'].elements['incDecCbo'].value != ''){
			...
			FeeWpayRwIncDecOtherService.countDataList
			(
				'<%=userId%>',
				'<%=ouCode%>',
				parseInt(DWRUtil.getValue("year")),
	...
	var rwIncDecOther = {keySeq:null,ouCode:null,yearPr:null,periodPr:null,empCode:null,groupCode:null,incDecCode:null,
						 ...
						 seqData:null,startDate:null,endDate:null,confirmFlag:null,creBy:null,updBy:null,creDate:null,updDate:null};
	var allRowUpdate = 0;
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwIncDecOtherService.addList(rwIncDecOther, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWpayRwIncDecOtherService.addList(rwIncDecOther, false);
								
							}else{
								if( allRowUpdate == myUpdate.length )
										FeeWpayRwIncDecOtherService.addList(rwIncDecOther, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
									else
										FeeWpayRwIncDecOtherService.addList(rwIncDecOther, false);
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYMT008] บันทึกรายการรับอื่น ๆ
		</td>
	</tr>
</table>
