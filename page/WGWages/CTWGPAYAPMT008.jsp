   ...
   /* function whenFetchEmployeeToCallback(data)
	...
	{	
		...
		if(document.forms['searchForm'].elements['incDecCbo'].value != null && document.forms['searchForm'].elements['incDecCbo'].value != ''){
			...
			//alert('query');
			// Query by Criteria
			FeeWgRwIncDecOtherService.findByCriteriaListApprove
			(
				'<%=userId%>',
				'<%=ouCode%>',
	...
	var rwIncDecOther = {keySeq:null,ouCode:null,yearPr:null,periodPr:null,empCode:null,groupCode:null,incDecCode:null,
						 ...
						 seqData:null,startDate:null,endDate:null,confirmFlag:null,creBy:null,updBy:null,creDate:null,updDate:null,approveFlag:null,approveBy:null,approveDate:null};
	var allRowUpdate = 0;
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWgRwIncDecOtherService.addListApprove(rwIncDecOther, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWgRwIncDecOtherService.addListApprove(rwIncDecOther, false);
								
							}else{
								if( allRowUpdate == myUpdate.length )
										FeeWgRwIncDecOtherService.addListApprove(rwIncDecOther, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
									else
										FeeWgRwIncDecOtherService.addListApprove(rwIncDecOther, false);
	 ...
	 function whenFetchEmployeeDetailInRowCallback(data)
	 {
		...
		function countData()
		{
			...
			//if(document.forms['searchForm'].elements['incDecCbo'].value != null && document.forms['searchForm'].elements['incDecCbo'].value != ''){
				...
				// Query by Criteria
				FeeWgRwIncDecOtherService.countDataApprove
				(
					'<%=userId%>',
					'<%=ouCode%>',
					parseInt(DWRUtil.getValue("year")),
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYAPMT008] บันทึกรับรองรายการรับอื่น ๆ (ลูกจ้างเหมา)
		</td>
	</tr>
</table>
...
<table  width="800" border="0" cellspacing="0" cellpadding="0" align="center">
	<tr>
		...
		<table id="table" width="1050"  border="1" bordercolor="#6699CC" cellpadding="0" cellspacing="0">
			...
			<td id="totalAmt" align="center"><input type="text"  name="totalAmt" readonly="readonly" maxlength="9" align="center"  onchange="checkValue(this);" style="text-align:right;width:100%;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
			<td  align="center" id="start">
				<div>
				<select name="startMonth" onchange="validateMonthYear(this);">
				<option value="" disabled="disable" selected="selected"  ></option>
			...
			<td  align="center" id="end">
				...
				/
				<input type="text"  name="endYear" readonly="readonly"  maxlength="4"    style="text-align:right;width:40px;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);validateMonthYear(this);"/>
				</div>
			</td>
			<td id="seqData" align="center"><input type="text"  name="seqData" readonly="readonly" maxlength="3" style="text-align:right;width:100%;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/><input type="hidden"  name="keySeq" /></td>
