...
<html>
<head>
<title>ยืนยันการบันทึกหักรับค่าจ้างไม่เต็มเดือน</title>
<!-- Include -->
...
var statusConfirm = 'N';
	function init()
	{
		...
		// FeeWgPrEmployeeTextService.deleteIncomeError
		//(
		//	'<%=ouCode%>',
		...
		//FeeWgPrEmployeeTextService.deleteDeductError
		//(
		//	'<%=ouCode%>',
		//	DWRUtil.getValue("year"),
		//	DWRUtil.getValue("period"),
	...
	function whenShowDataTable()
	{
		...
		DWRUtil.useLoadingMessage("Loading ...");
		//alert("test");
		FeeWgPrEmployeeTextService.updateConfirmVinai2
		(
			'<%=userId%>',
			'<%=ouCode%>',
			DWRUtil.getValue("year"),
	...
	function whenShowDataTable2()
	{
		...
		FeeWgPrEmployeeTextService.updateConfirmDeduct
		(
			'<%=userId%>',
			'<%=ouCode%>',
			DWRUtil.getValue("year"),
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYMT013 ] ยืนยันการบันทึกเงินหักค่าจ้างไม่เต็มเดือน
		</td>
	</tr>
</table>
