   ...
   /* function whenFetchEmployeeToCallback(data)
	...
	{	
		myUpdate = new Array();
			...
			FeeWpayRwPremiumService.findByCriteriaList
			(
				'<%=userId%>',
				'<%=ouCode%>',
    ...
    function countData(){
		...
		FeeWpayRwPremiumService.countDataList(
				'<%=userId%>',
				'<%=ouCode%>',
				parseInt(DWRUtil.getValue("year")),
				parseInt(DWRUtil.getValue("period")),
	...
	var feeWpayRwPremium = {keySeq:null, ouCode:null, yearPr:null, periodPr:null,empCode:null,yearWork:null,periodWork:null,codeSeq:null,morDay:null,aftDay:null,evnDay:null,morHour:null,aftHour:null,
					 evnHour:null,amtMor:null,amtAft:null,amtEvn:null,flagPr:null,confirmFlag:null,seqData:null,updBy:null,updDate:null,creBy:null,creDate:null,approveFlag:null};
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwPremiumService.addList(feeWpayRwPremium, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWpayRwPremiumService.addList(feeWpayRwPremium, false);
					
								}else{
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwPremiumService.addList(feeWpayRwPremium, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
								else
									FeeWpayRwPremiumService.addList(feeWpayRwPremium, false);
		...
		if(empNull){
			...
			for(var i=oRows.length-1;i > num;i--)
			{
					...
					if(oRows[i].cells["aftDay"].childNodes[0].value != ''){
						feeWpayRwPremium.aftDay = oRows[i].cells["aftDay"].childNodes[0].value;
					}else{
						feeWpayRwPremium.aftDay = null;
					}	
					...
					feeWpayRwPremium.approveFlag = 'N';
					feeWpayRwPremium.updBy = '<%=userId%>';
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYMT005 ] บันทึกเงินเพิ่มพิเศษเป็นกะ
		</td>
	</tr>
</table>
		...
		<table id="table" width="900"  border="1" bordercolor="#6699CC" cellpadding="0" cellspacing="0">
			<thead style="text-align: center">
				<tr CLASS="TABLEBULE2" style="height: 30px;">
					<th CLASS="TABLEBULE2" style="width:40px" rowspan="2" align="center">ลบ</th>
			...
			<td id="evnHour" align="center" ><input type="text"  name="evnHour"  maxlength="3" align="center" style="text-align:right;width:100%;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
