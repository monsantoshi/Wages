...
<script type="text/javascript">
   ...
   /* function whenFetchEmployeeToCallback(data)
	...
	function whenGenerate(){
		...
		FeeWpayRwPremiumService.generate
			(
				'<%=userId%>',
				'<%=ouCode%>',
	...
	{	
		myUpdate = new Array();
			...
			(
				...
				empFromCbo,
				empToCbo,
				{callback:whenListDataTableHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร1');}}
			);
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwPremiumService.addList(rwPremium, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWpayRwPremiumService.addList(rwPremium, false);
					
								}else{
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwPremiumService.addList(rwPremium, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
								else
									FeeWpayRwPremiumService.addList(rwPremium, false);
	...
	function insertNewData()
	{
		...
		if(empNull){
			...
			for(var i=oRows.length-1;i > num;i--)
			{
					...
					rwPremium.confirmFlag = 'N';
					rwPremium.updBy = '<%=userId%>';
					rwPremium.creBy = '<%=userId%>';
					rwPremium.creDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
...
<table  width="800" border="0" cellspacing="0" cellpadding="0" align="center">
	...
	<table width="770" align="center" cellpadding="2" cellspacing="0">
	    <tr><td align="right"><input type="hidden" name="page" value="<%=pageEdit%>"> 
	    <input type="hidden" name="maxPage"> <input type="hidden" name="countData">
				    ...
				    if(tab.rows.length>1){
						for(i=0; i<tab.rows.length; i++){
							...
							if (chk[i].checked){
									...
									FeeWpayRwPremiumService.deleteRwPremium(rwPremium, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
									chDelete = true;
								}
