...
<script type="text/javascript" src="script/editableSelectBox.js"></script>
...
<!--
	...
	var rwVinai = {	keySeq:null,ouCode:null,yearPr:null,periodPr:null,empCode:null,
					...
					cutSalYear:null,cutSalMonth:null,cutSalPercent:null,confirmFlag:null,approveFlag:null,
	...
	function insertNewData()
	{
			...
			if(oRows.length > 3)
			{
				...
				{
				   var periodWWork = parseInt(oRows[i].cells["cutSalMonth"].childNodes[0].value);
	    		   var yearWWork = oRows[i].cells["cutSalYear"].childNodes[0].value;
	    		   var periodPWork = parseInt(oRows[i].cells["absMonth1"].childNodes[0].value);
	    		   var yearPWork = oRows[i].cells["absYear1"].childNodes[0].value;
		  		   var section = '<%=sectionInsert%>';
		           var yearInt = '<%=yearInsert%>';
		        ...
		        if ((yearWWork <=yearInt &&  periodWWork != null )||(yearPWork<=yearInt && periodPWork != null)) {
				  ...
				  else  if((yearWWork<=yearInt && periodWWork  != null ) && (oRows[i].cells["absYear1"].childNodes[0].value=='' && oRows[i].cells["absMonth1"].childNodes[0].value == '')){
					rwVinai.ouCode = '<%=ouCodeInsert%>';
					rwVinai.yearPr = '<%=yearInsert%>';
					rwVinai.periodPr = '<%=periodInsert%>';
					rwVinai.empCode = DWRUtil.getValue("empCbo");
				  ...
				  else if ((oRows[i].cells["cutSalYear"].childNodes[0].value=='' && oRows[i].cells["cutSalMonth"].childNodes[0].value == '') && (yearPWork<=yearInt && periodPWork != null)){
					rwVinai.ouCode = '<%=ouCodeInsert%>';
					rwVinai.yearPr = '<%=yearInsert%>';
					rwVinai.periodPr = '<%=periodInsert%>';
					rwVinai.empCode = DWRUtil.getValue("empCbo");
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYIN009 ] บันทึกรายการหักกรณีวินัย ตัดค่าจ้าง และขาดงาน
		</td>
	</tr>
</table>
				...
				<table id="table"  bordercolor="#4A4A4A" align="center" style="text-align: center;width:800px" border="1" cellpadding="0" cellspacing="0" >
					<thead style="text-align: center">
						<tr CLASS="TABLEBULE2">
							...
							<th CLASS="TABLEBULE2" style="width:300px" colspan="3" rowspan="1" align="center"><center>ตัดค่าจ้าง</center></th>
							<th CLASS="TABLEBULE2" style="width:300px" colspan="4" rowspan="1" align="center"><center>ขาดงาน</center></th>
						</tr>
						<tr CLASS="TABLEBULE2"  >
							<th CLASS="TABLEBULE2" colspan="2" align="center">ปี/เดือน</th>
							<th CLASS="TABLEBULE2" style="width:80px" align="center">จำนวน (%)</th>
							<th CLASS="TABLEBULE2" colspan="2" align="center">ปี/เดือน</th>
							<th CLASS="TABLEBULE2" style="width:80px"align="center">จำนวน(วัน)</th>												
						</tr>
					</thead>
						<tr id="temprow" style="visibility:hidden;position:absolute">
							...
							<td id="cutSalPercent" ><input type="text"  name=cutSalPercent  maxlength="9" style="text-align:center;width:100%;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
							...
							<td id="absDay1" ><input type="text"  name=absDay1  maxlength="6" style="text-align:center;width:100%;" onchange="checkDay(this);" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
							</tr>
					</table>
				</div>
