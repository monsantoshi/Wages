   ...
   /* function whenFetchEmployeeToCallback(data)
	...
	{	
		...
		//alert(document.forms['searchForm'].elements['incDecCbo'].value);
			...
			//alert('query');
			// Query by Criteria
			FeeWpayRwVinaiService.findByCriteriaListApprove
			(
				'<%=userId%>',
				'<%=ouCode%>',
	...
	var feeWpayRwVinai = {	keySeq:null,ouCode:null,yearPr:null,periodPr:null,empCode:null,
			...
			if(canSave){
				if(empNull){
					...
					for(var i=0; i<tab.rows.length; i++){
						update = false;
						row = tab.rows[i];
						feeWpayRwVinai.keySeq = parseInt(keySeq[i].value);
						feeWpayRwVinai.codeSeq = codeSeq[i].value;
						if(tab.rows.length == 1){
							feeWpayRwVinai.flagPr = flagPr.value;
						}else{
							feeWpayRwVinai.flagPr = flagPr[i].value;
						}
						if (empCode[i].value != '' && empCode[i].value != null){
							feeWpayRwVinai.empCode  = empCode[i].value;
						}
						else{
							feeWpayRwVinai.empCode  = null;
						}
						
						if (name[i].value != '' && name[i].value != null){
							feeWpayRwVinai.name  = name[i].value;
						}
						else{
							feeWpayRwVinai.name  = null;
						}
						if (yearWork[i].value != ''&& yearWork[i].value != null){
							feeWpayRwVinai.yearWork  = yearWork[i].value;
						}
						else{
							feeWpayRwVinai.yearWork  = null;
						}
						
						if (periodWork[i].value != '' && periodWork[i].value != null){
							feeWpayRwVinai.periodWork  = periodWork[i].value;
						}
						else{
							feeWpayRwVinai.periodWork  = null;
						}
						
						
						feeWpayRwVinai.approveFlag = approveF[i].value;
						feeWpayRwVinai.approveBy = '<%=userId%>';
						//feeWpayRwVinai.updBy = '<%=userId%>';
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwVinaiService.addListApprove(feeWpayRwVinai, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWpayRwVinaiService.addListApprove(feeWpayRwVinai, false);
					
								}else{
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwVinaiService.addListApprove(feeWpayRwVinai, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
								else
									FeeWpayRwVinaiService.addListApprove(feeWpayRwVinai, false);
	...
	function insertNewData()
	{
		...
		if(empNull){
			...
			{
					insertStatus = true;
					feeWpayRwVinai.keySeq = null;
					feeWpayRwVinai.ouCode = '<%=ouCode%>';
					feeWpayRwVinai.yearPr = parseInt($("year").value);
					feeWpayRwVinai.periodPr = parseInt($("period").value);
					feeWpayRwVinai.empCode = oRows[i].cells["empCode"].childNodes[0].value;
					feeWpayRwVinai.name = oRows[i].cells["name"].childNodes[0].value;
					feeWpayRwVinai.codeSeq = oRows[i].cells["name"].childNodes[1].value;
					feeWpayRwVinai.flagPr = oRows[i].cells["flagPr"].childNodes[0].value;
					feeWpayRwVinai.yearWork = oRows[i].cells["yearWork"].childNodes[0].value;
					feeWpayRwVinai.periodWork = oRows[i].cells["periodWork"].childNodes[0].value;
					
					
					
					
					feeWpayRwVinai.approveFlag = oRows[i].cells["approveF"].childNodes[0].value;
					feeWpayRwVinai.approveBy = '<%=userId%>';
					//feeWpayRwVinai.approveDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
					//feeWpayRwVinai.confirmFlag = 'N';
					//feeWpayRwVinai.updBy = '<%=userId%>';
					//feeWpayRwVinai.creBy = '<%=userId%>';
					//feeWpayRwVinai.creDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
	 ...
	 function whenFetchEmployeeDetailInRowCallback(data)
	 {
		...
		function countData()
		{
			...
			//if(document.forms['searchForm'].elements['incDecCbo'].value != null && document.forms['searchForm'].elements['incDecCbo'].value != ''){
				...
				// Query by Criteria
				FeeWpayRwVinaiService.countDataApprove
				(
					'<%=userId%>',
					'<%=ouCode%>',
					parseInt(DWRUtil.getValue("year")),
		...
		<table id="table" width="1100"  border="1" bordercolor="#6699CC" cellpadding="0" cellspacing="0">
			<thead style="text-align: center">
				<tr CLASS="TABLEBULE2" style="height: 30px;">
					<th CLASS="TABLEBULE2" rowspan="2" style="width:90px">ประเภทรายการ</th>
					<th CLASS="TABLEBULE2" rowspan="2" style="width:120px">เลขประจำตัว</th>
					<th CLASS="TABLEBULE2" rowspan="2" style="width:250px">ชื่อ - นามสกุล</th>
					<th CLASS="TABLEBULE2" colspan="2" rowspan="2" align="center">ปี/งวดที่ทำงาน</th>
					<th CLASS="TABLEBULE2" style="width:200px" colspan="3" rowspan="1" align="center"><center>ตัดเงินเดือน</center></th>
					<th CLASS="TABLEBULE2" style="width:200px" colspan="3" rowspan="1" align="center"><center>ขาดงาน</center></th>
				    ...
				    if(tab.rows.length>1){
						for(i=0; i<tab.rows.length; i++){
							...
							if (chk[i].checked){
									...
									FeeWpayRwVinaiService.deleteRwVinai(rwVinai, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร7');}});
									chDelete = true;
								}
						...
						if(tab.rows.length==1){
							...
							if (chk[0].checked){
									...
									FeeWpayRwVinaiService.deleteRwVinai(rwVinai, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร8');}});
									chDelete = true;
								}
