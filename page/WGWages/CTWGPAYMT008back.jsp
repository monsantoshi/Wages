...
<%
	...
	//System.out.println("ConfirmRwIncDecOther : " + session.getAttribute("ConfirmRwIncDecOther").getClass().getName());
	//String confirm = (String)session.getAttribute("ConfirmRwIncDecOther");
	//System.out.println("ConfirmRwIncDecOther : " + confirm);
	
	String periodWork = String.valueOf(x);
%>
<html>
<head>
<title>บันทึกรายการรับอื่น ๆ</title>
<!-- Include -->
...
<script type="text/javascript" src="dwr/interface/FeeWgRwIncDecOtherService.js"></script>
...
<script type="text/javascript">
    ...
    function onLoadYearSectionCallback()
    {
		...
		$("confirm").value = "<c:out value='${ConfirmRwIncDecOther}' /> "  ;//data.confirm;
		chkMainClose();
		...
		//PnEmployeeService.findPrEmpBySecurity('<%=userId%>','<%=ouCode%>',$("year").value,$("period").value, {callback:onLoadEmployeeCallback});
		 ...
		 if(<c:out value='${ConfirmRwIncDecOther}' />){
   ...
   /* function whenFetchEmployeeToCallback(data)
	...
	function init()
	{
		...
		function chkMainClose(){
				FeeWgPrPeriodLineService.findPeriodLine('<%=ouCode%>',$("year").value,$("period").value, '<%=userId%>', {callback:whenchkMainCloseCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		    }
	...
	function check(){
		if(document.forms['searchForm'].elements['incDecCbo'].value == '08' && !<c:out value='${ConfirmRwIncDecOther}' />){
	...
	function whenGenerate(){
		...
		FeeWgRwIncDecOtherService.generate
			(
				'<%=userId%>',
				'<%=ouCode%>',
				parseInt(DWRUtil.getValue("year")),
				parseInt(DWRUtil.getValue("period")),
				{callback:whenGenerateHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}}
		);
	
	}
	function whenGenerateHandler(data){
		whenShowDataTable();
	}
	...
	{	
		...
		if(document.forms['searchForm'].elements['incDecCbo'].value != null && document.forms['searchForm'].elements['incDecCbo'].value != ''){
			...
			FeeWgRwIncDecOtherService.findByCriteriaList
			(
				'<%=userId%>',
				'<%=ouCode%>',
				parseInt(DWRUtil.getValue("year")),
	...
	var cellFuncs = [
		...
		//function(data) { return writeStart(data.startDate,data.keySeq);},
		function(data) { 
	...
	function countData()
		...
		//if(document.forms['searchForm'].elements['incDecCbo'].value != null && document.forms['searchForm'].elements['incDecCbo'].value != ''){
			...
			FeeWgRwIncDecOtherService.countDataList
			(
				'<%=userId%>',
				'<%=ouCode%>',
				parseInt(DWRUtil.getValue("year")),
	...
	var rwIncDecOther = {keySeq:null,ouCode:null,yearPr:null,periodPr:null,empCode:null,groupCode:null,incDecCode:null,
						 ...
						 seqData:null,startDate:null,endDate:null,confirmFlag:null,creBy:null,updBy:null,creDate:null,updDate:null,approveFlag:null};
	var allRowUpdate = 0;
	...
	function onUpdate(){
		...
		for(var a=aRows.length-1;a > num;a--){
				...
				if (aRows[a].cells["totalAmt"].childNodes[0].value == null || aRows[a].cells["totalAmt"].childNodes[0].value == ''){
					canSave = false ;
					break;
				}
		...
		if(tab.rows.length > 0){
			...
			for(var c=0; c<tab.rows.length; c++){
				...
				if(totalAmt[c].value == null || totalAmt[c].value == ''){
					canSave = false ;
					break;
				}
			...
			if(canSave){
				if(empNull){
					...
					for(var i=0; i<tab.rows.length; i++){
						...
						rwIncDecOther.periodWork  = periodWork[i].value;
						
						if (yearWork[i].value != ''){
							rwIncDecOther.yearWork  = yearWork[i].value;
						}
						else{
							rwIncDecOther.yearWork  = null;
						}
						...
						if(startMonth[i].value != '' && startYear[i].value != '' && endMonth[i].value != '' && endYear[i].value != '' ){
							...
							//alert(startMonth[i].value+' ::: '+startYear[i].value+' ::: '+convertArrayToDate(arrStart));
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWgRwIncDecOtherService.addList(rwIncDecOther, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWgRwIncDecOtherService.addList(rwIncDecOther, false);
								
							}else{
								if( allRowUpdate == myUpdate.length )
										FeeWgRwIncDecOtherService.addList(rwIncDecOther, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
									else
										FeeWgRwIncDecOtherService.addList(rwIncDecOther, false);
	...
	function insertNewData()
	{
		...
		if(empNull){
			...
			for(var i=oRows.length-1;i > num;i--)
			{
					...
					rwIncDecOther.yearWork = oRows[i].cells["yearWork"].childNodes[0].value;
					rwIncDecOther.periodWork = oRows[i].cells["periodWork"].childNodes[0].value;
					...
					if(oRows[i].cells["start"].childNodes[0].childNodes[0].value != '' && oRows[i].cells["start"].childNodes[0].childNodes[2].value != '' && oRows[i].cells["end"].childNodes[0].childNodes[0].value != '' && oRows[i].cells["end"].childNodes[0].childNodes[2].value != '' ){
						var startMonth = oRows[i].cells["start"].childNodes[0].childNodes[0].value;
						var startYear = oRows[i].cells["start"].childNodes[0].childNodes[2].value;
						var endMonth = oRows[i].cells["end"].childNodes[0].childNodes[0].value;
						var endYear = oRows[i].cells["end"].childNodes[0].childNodes[2].value;
						
						var arrStart = new Array();
						arrStart[0] = startMonth;
						arrStart[1] = startYear;
						
						
						if(arrStart[0].length == 1){
								sst = "01/0"+startMonth+"/"+startYear;
							}else{
								sst = "01/"+startMonth+"/"+startYear;
							}
						rwIncDecOther.startDate = getDateFromFormat(sst,"dd/MM/yyyy");
						
						//alert('y month : ' + convertArrayToDate(arrStart) );
						//rwIncDecOther.startDate = convertArrayToDate(arrStart);
						//alert('y year : ' + rwIncDecOther.startDate );
						...
						if(arrEnd[0].length == 1){
								sed = "01/0"+endMonth+"/"+endYear;
							}else{
								sed = "01/"+endMonth+"/"+endYear;
							}
						rwIncDecOther.endDate = getDateFromFormat(sed,"dd/MM/yyyy");
						//alert('y month : ' + convertArrayToDate(arrEnd) );
						//rwIncDecOther.endDate = convertArrayToDate(arrEnd);
						//alert('y year : ' + rwIncDecOther.endDate );
					}else{
						rwIncDecOther.startDate = null;
						rwIncDecOther.endDate  = null;
					}
					...
					rwIncDecOther.confirmFlag = 'N';
					rwIncDecOther.approveFlag = 'N';
					rwIncDecOther.updBy = '<%=userId%>';
					rwIncDecOther.creBy = '<%=userId%>';
					rwIncDecOther.creDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
	...
	function whenChengIncDec(){
		...
		if(str == '08' && !<c:out value='${ConfirmRwIncDecOther}' />){
			document.forms['searchForm'].elements['generate'].disabled = false;
		}else{
			document.forms['searchForm'].elements['generate'].disabled = true;
		}
		...
		if(amt != null && amt != ''){
			if(strIncDec == '08'){
				if(amt == '800'){
					...
					FeeWgPrIncomeDeductService.findMaxIncDecCode('<%=ouCode%>','1',strIncDec, {callback:whenCheckAmtTotalCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	...
	function checkValueInRowUpdate(data){
		...
		if(totalAmtValue != null && totalAmtValue != ''){
			if(strIncDec == '08'){
				...
				FeeWgPrIncomeDeductService.findMaxIncDecCode('<%=ouCode%>','1',strIncDec, {callback:whenCheckAmtTotalInRowUpdateCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	...
	function whenSelectEmpOptionInRow(object)
	 ...
	 function whenFetchEmployeeDetailInRowUpdate(empCode)
	 {
		...
		FeeWgPnEmployeeService.findByEmpCodeDetailM('<%=userId%>',empCode, '<%=ouCode%>', parseInt(DWRUtil.getValue("year")), parseInt(DWRUtil.getValue("period")), {callback:whenFetchEmployeeDetailInRowUpdateCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	 }
	 ...
	 function whenFetchEmployeeDetailInRow(empCode)
	 {
		DWRUtil.useLoadingMessage("Loading ...");
		//alert(empCode+' : '+<%=ouCode%>+' : ' + DWRUtil.getValue("year")+' : '+ DWRUtil.getValue("period"))
		FeeWgPnEmployeeService.findByEmpCodeDetailM('<%=userId%>',empCode, '<%=ouCode%>', parseInt(DWRUtil.getValue("year")), parseInt(DWRUtil.getValue("period")), {callback:whenFetchEmployeeDetailInRowCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	 }
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYMT008] บันทึกรายการรับอื่น ๆ
		</td>
	</tr>
</table>
<table width="770" border="0" align="center" cellspacing="1">
 	...
 	<tr>
		<td class="font-field" align="right">ประเภทเงินได้&nbsp;</td>
		<td align="left">
		...
		<td><input type="Button" value="Generate" name="generate" class=" button " onclick="whenGenerate();"  disabled="disabled"/></td>
		<td></td>
	</tr>
...
<table  width="800" border="0" cellspacing="0" cellpadding="0" align="center">
	<tr>
		...
		<table id="table" width="1050"  border="1" bordercolor="#6699CC" cellpadding="0" cellspacing="0">
			...
			<td id="totalAmt" align="center"><input type="text"  name="totalAmt"  maxlength="9" align="center"  onchange="checkValue(this);" style="text-align:right;width:100%;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
			<td  align="center" id="start">
				...
				/
				<input type="text"  name="startYear"  maxlength="4"   style="text-align:right;width:40px;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);validateMonthYear(this);"/>
				</div>
				</td>
			<td  align="center" id="end">
				...
				/
				<input type="text"  name="endYear"  maxlength="4"    style="text-align:right;width:40px;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);validateMonthYear(this);"/>
				</div>
			</td>
				    ...
				    if(tab.rows.length>1){
						for(i=0; i<tab.rows.length; i++){
							...
							if (chk[i].checked){
									...
									FeeWgRwIncDecOtherService.deleteRwIncDecOther(rwIncDecOther, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
									chDelete = true;
								}
