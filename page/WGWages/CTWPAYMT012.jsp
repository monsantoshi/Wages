...
var statusConfirm = 'N';
	function init()
	{
		...
		//FeeWpayPrPeriodLineService.getDefaultYearAndSection('<%=ouCode%>', '<%=userId%>', {callback:onLoadYearSectionCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		...
		(
			...
			'<%= userId %>',
			{callback:onLoadIncDecCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}}
		);
	...
	function whenShowDataTable()
	{
		...
		//alert("test");
		FeeWpayPrEmployeeTextService.updateConfirmIncome
		(
			'<%=userId%>',
			'<%=ouCode%>',
			DWRUtil.getValue("year"),
		...
		if (statusConfirm == 'Y'){
			...
			(
				confirmDataVo,
				{callback:whenInsertRwConfirmDataHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}}
			);
		}
	}
	...
	function whenInsertRwConfirmDataHandler(data){
		...
		btn.disabled = true;
		alert("ยืนยันเงินได้/เงินหัก เรียบร้อยแล้ว");
	}
	...
	function whenShowDataTable2()
	{
		...
		FeeWpayPrEmployeeTextService.updateConfirmDeduct
		(
			'<%=userId%>',
			'<%=ouCode%>',
			DWRUtil.getValue("year"),
			DWRUtil.getValue("period"),
			{callback:whenListDataTable2Handler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}}
		);
	}
	...
	//function chkMasterClose(){
	//			FeeWpayRwConfirmDataService.findConfirmMaster('<%=ouCode%>',$("year").value,$("period").value ,'<%= userId %>', {callback:whenchkMasterCloseCallback});
	//	    }
			
	//		function whenchkMasterCloseCallback(data){
		//	    alert(data.flag2);
		//		if(data.flag2 == '0'){
		//			alert('ไม่สามารถดำเนินการต่อได้ ยังไม่ได้ยืนยันข้อมูลหลัก');
		//			window.history.back()
		//		}
		//	}
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYMT012 ] ยืนยันการบันทึกรายการเงินได้/เงินหัก ลูกจ้าง
		</td>
	</tr>
</table>
...
<tr>
	<td valign="top">
	<FIELDSET>
    		<LEGEND>รายการเงินได้/เงินหัก</LEGEND>
    		<br>
