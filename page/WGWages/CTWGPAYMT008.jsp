...
<script type="text/javascript">
	...
	function whenShowDataTable()
	{
		if(document.forms['searchForm'].elements['incDecCbo'].value != null && document.forms['searchForm'].elements['incDecCbo'].value != ''){
			...
			// Query by Criteria
			FeeWgRwIncDecOtherService.findByCriteria
			(
				'<%=userId%>',
				'<%=ouCode%>',
	...
	var rwIncDecOther = {keySeq:null,ouCode:null,yearPr:null,periodPr:null,empCode:null,groupCode:null,incDecCode:null,
						 ...
						 seqData:null,confirmFlag:null,creBy:null,updBy:null,creDate:null,updDate:null,approveFlag:null};
	var allRowUpdate = 0;
	...
	function insertNewData()
	{
		...
		if(empNull){
			...
			for(var i=oRows.length-1;i > num;i--)
			{
					...
					rwIncDecOther.creDat = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
	...
	function checkValue(object){
		...
		if(amt != null && amt != '' && amt >= 0){
			FeeWgPrIncomeDeductService.findMaxIncDecCode('<%=ouCode%>','2',strIncDec, {callback:whenCheckAmtTotalCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		}else{
	...
	function checkValueInRowUpdate(data){
		...
		if(totalAmt != null && totalAmt != '' && totalAmt >= 0){
				...
				FeeWgPrIncomeDeductService.findMaxIncDecCode('<%=ouCode%>','2',strIncDec, {callback:whenCheckAmtTotalInRowUpdateCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		}else{
			alert('กรุณากรอกจำนวนเงิน');
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYMT008 ] บันทึกเงินรับอื่น ๆ
		</td>
	</tr>
</table>
<table width="770" border="0" align="center" cellspacing="1">
 	...
 	<tr>
		<td class="font-field" align="right">ประเภทเงินรับ&nbsp;</td>
		<td align="left">
		...
		<table id="table" width="865"  border="1" bordercolor="#6699CC"   cellpadding="0" cellspacing="0">
			<thead style="text-align: center">
				<tr CLASS="TABLEBULE2" style="height: 30px;">
					<th CLASS="TABLEBULE2" style="width:40px">ลบ</th>
			...
			<tr id="temprow" style="visibility:hidden;position:absolute">
				...
				<td id="flagPr" width="50" align="center">
					<select  name="type" >
						<option value="N"  >ปกติ</option>
						<option value="A" >ปรับปรุงรายการรับ</option>
				</select>
			</td>
			<td id="empCode" align="center"><input type="text" name="empCode" maxlength="13" style="width:100%" onchange="whenSelectEmpOptionInRow(this);"  onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"></td>
			...
			<td id="totalAmt" align="center"><input type="text"  name="totalAmt" onchange="checkValue(this);"  maxlength="9" align="center" style="text-align:right;width:100%;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
			<td  align="center" id="start">
				<div align="center">
					...
					/
					<input type="text"  name="startYear"  maxlength="4"   align="center" style="text-align:right;width:40px;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);validateMonthYear(this);"/>
				</div>
				</td>
			<td  align="center" id="end">
				<div align="center">
					...
					/
					<input type="text"  name="endYear"  maxlength="4"  align="center" style="text-align:right;width:40px;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);validateMonthYear(this);"/>
				</div>
			</td>
