...
<script type="text/javascript">
	...
	function init()
	{
		...
		FeeWpayPrEmployeeTextService.deletePrEmployeeTextError
		(
			'<%=ouCode%>',
	...
	{
		allOrder = 0;
		FeeWpayPrEmployeeTextService.confirmData
		(
			'<%=ouCode%>',
			DWRUtil.getValue("year"),
			DWRUtil.getValue("period"),
	...
	function whenCountDataIHandler(data)
	{
		...
		if (statusConfirm == 'Y'){		
			if(allOrder==0)
			{
				...
				FeeWpayRwConfirmDataService.insertRwConfirmData01
					(
						confirmDataVo,
			...
			}else
			{
				DWRUtil.useLoadingMessage("Loading ...");
					...
					FeeWpayPrEmployeeTextService.updateConfirmData
					(
						'<%=ouCode%>',
						DWRUtil.getValue("year"),
						DWRUtil.getValue("period"),
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYMT002 ] ยืนยันการเปลี่ยนแปลงข้อมูลหลักลูกจ้าง
		</td>
	</tr>
</table>
