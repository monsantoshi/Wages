	...
	<head>
		...
		var canSave = true;
		    ...
		    function whenContractCallback(data)
		   {
		    ...
		    var cellFuncs = [
			             		function(data) { return writeApp("inactiveD",data.inactiveD,data.keySeq);},
			             		...
			             		function(data) { return writeText("otRate",data.otRate,4,"right",data.keySeq);},
								...
								function(data) { 
			             		...
			             		function(data) { if(data.note != null && data.note != '') {
			             			return writeTextNote("note",data.note,200,"left","keySeq",data.keySeq);
			             		}else {
			             			return writeTextNote("note","",200,"left","keySeq",data.keySeq);
			             		}
									}];
			...
			function compareStDate(object){
			 	//document.forms['searchForm'].elements['refNo'].focus();
			  			while (object.tagName !=  'TR')
							...
							//var inactiveFlag = oRows[rowToCompare].cells["inactive"].childNodes[0].value;
							...
							if(stDate != null && stDate != '' && endDate != null && endDate != ''){
								...
								if (compareDates(stDate,"dd/MM/yyyy",endDate,"dd/MM/yyyy") == 1) {
									...
									document.forms['searchForm'].elements['insertData'].disabled = false;
									document.forms['searchForm'].elements['deleteData'].disabled = false;
									document.forms['searchForm'].elements['confirmData'].disabled = false;
								//	alert('in this');
			 ...
			 function whenQueryConBefore()
			    ...
			    var feeContractDetails = {keySeq:null,ouCode:null,yearCon:null,codeSeq:null,contractNo:null,empCode:null,
						dutyCode:null, inactive:null,startDate:null,endDate:null,salary:null,extraIncome:null,warfee:null,otRate:null,note:null,creBy:null,updBy:null,creDate:null,updDate:null};
				...
				if(tab.rows.length > 0){
					...
					var inactiveD    = frm.elements["inactiveD"];
					//var dutyCode    = frm.elements["dutyCode"];
					...
					var otRate 		= frm.elements["otRate"];
					var extraIncome 	= frm.elements["extraIncome"];
					var note 	= frm.elements["note"];
						...
						if(empNull){
							...
							for(var i=0; i<tab.rows.length; i++){
								...
								if(tab.rows.length == 1){
									feeContractDetails.inactive = inactiveD[i].value;
								}else{
									feeContractDetails.inactive = inactiveD[i].value;
								}
								if (inactiveD[i].value != '' && inactiveD[i].value != null){
									feeContractDetails.inactive  = inactiveD[i].value;
								}
								else{
									feeContractDetails.inactive  = null;
								}
								...
								if(startDate[i].value != ''){
									feeContractDetails.startDate = getDateFromFormat(startDate[i].value,"dd/MM/yyyy");
								}else{
									feeContractDetails.startDate  = getDateFromFormat($("scontactDate").value,"dd/MM/yyyy");
								}
								
								if(endDate[i].value != ''){
									feeContractDetails.endDate = getDateFromFormat(endDate[i].value,"dd/MM/yyyy");
								}else{
									feeContractDetails.endDate  = getDateFromFormat($("econtactDate").value,"dd/MM/yyyy");
								}
								if (salary[i].value !=''){
								...
								if (otRate[i].value !=''){
									feeContractDetails.otRate  = otRate[i].value;
								}
								else{
									feeContractDetails.otRate  = null;
								}
								if (warfee[i].value !=''){
								...
								if (note[i].value != '' && note[i].value != null){
									feeContractDetails.note  = note[i].value;
								}
								else{
									feeContractDetails.note  = null;
								}
								feeContractDetails.contractNo = $("contractNo").value;
								feeContractDetails.codeSeq = parseInt($("codeSeqAct").value);
								feeContractDetails.updBy = '<%=userId%>';
								feeContractDetails.updDate = getDateFromFormat(sed,"dd/MM/yyyy");
								feeContractDetails.dutyCode = parseInt($("dutyCode").value);
				...
				function insertNewData()
				{
					...
					if(empNull){
						...
						for(var i=oRows.length-1;i > num;i--)
						{
								...
								feeContractDetails.inactive = oRows[i].cells["inactiveD"].childNodes[0].value;
							    
								if(oRows[i].cells["startDate"].childNodes[0].value != null && oRows[i].cells["startDate"].childNodes[0].value != ''){
									feeContractDetails.startDate= getDateFromFormat(oRows[i].cells["startDate"].childNodes[0].value,"dd/MM/yyyy");
								}else{
									feeContractDetails.startDate= getDateFromFormat($("scontactDate").value,"dd/MM/yyyy");
								}
								if(oRows[i].cells["endDate"].childNodes[0].value != null && oRows[i].cells["endDate"].childNodes[0].value != ''){
									feeContractDetails.endDate = getDateFromFormat(oRows[i].cells["endDate"].childNodes[0].value,"dd/MM/yyyy");
								}else{
									feeContractDetails.endDate = getDateFromFormat($("econtactDate").value,"dd/MM/yyyy");
								}
								...
								if(oRows[i].cells["otRate"].childNodes[0].value != ''){
									feeContractDetails.otRate = oRows[i].cells["otRate"].childNodes[0].value;
								}else{
									feeContractDetails.otRate = null;
								}	
								...
								if(oRows[i].cells["note"].childNodes[0].value != ''){
									feeContractDetails.note = oRows[i].cells["note"].childNodes[0].value;
								}else{
									feeContractDetails.note = null;
								}	
								...
								//feeContractDetailsService.addList(feeContractDetails, {callback:onInsertCallback});
						...
						DWREngine.endBatch();
						if (!insertStatus){alert("บันทึกข้อมูลเรียบร้อย insert New");}
			...
			<tr>
				...
				<table id="table" width="1050" border="1" bordercolor="#6699CC" cellpadding="0" cellspacing="0">
					...
					<tr CLASS="TABLEBULE2" style="height: 30px;">
						<th CLASS="TABLEBULE2">สภาพจ้างตามสัญญา</th>
						...
						<th CLASS="TABLEBULE2" style="width: 100px"  align="center"><center>วดป.เริ่มงาน</center></th>
						<th CLASS="TABLEBULE2" style="width: 100px"  align="center"><center>วดป.สิ้นสุดงาน</center></th>
						<th CLASS="TABLEBULE2" style="width: 120px"   align="center"><center>หมายเหตุ</center></th>
					</tr>
					</thead>
					<tbody id="dataTable"></tbody>
						<tr id="temprow" style="visibility: hidden; position: absolute">
							<td id="inactiveD" width="50" ><select name="type">
															<option value="N" >ปกติ</option>
															<option value="R" >พ้นสภาพ</option>
															<option value="S" >เลิกจ้างมีโทษ</option>
															<option value="T" >ลบบันทึกผิด</option>
															</select>
							</td>
							...
							<td id="name" align="center"><input type="text"	name="name" readonly="readonly"	style="width: 100%; background-color: silver;"/><input type="hidden" name="codeSeqWork" /></td>
							<td id="warfee" align="center"><input type="text" name="warfee" maxlength="9" align="center" onchange="checkValue(this);" style="text-align: right; width: 100%;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);" /></td>
							...
							<td id="note" align="center"><input type="text" name="note" maxlength="100" align="left" style="text-align: left; width: 100%;"/></td>
						</tr>
				</table>
				</div></td>
				</tr>
