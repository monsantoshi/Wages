...
<!--
	...
	function whenQueryData()
	{
		...
		FeeWpayRwOvertimeService.findByEmpCodeDetail('<%=empID%>','<%=ouCodeEdit%>','<%=yearEdit%>','<%=periodEdit%>',{callback:whenQueryDetailHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	}
	...
	//	}
		...
		FeeWpayRwOvertimeService.findByEmpCodeList('<%=userId%>','<%=ouCodeEdit%>','<%=yearEdit%>','<%=periodEdit%>','<%=empID%>','<%=refNoEdit%>','<%=otTypeEdit%>',{callback:whenQueryListDataHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	...
	if(compareDateBeforSave()){
			...
			for(var i=0; i<tab.rows.length; i++){
					...
					if(workHour[i].value !=''){
						rwOvertime.workHour = workHour[i].value;
					}else{
					...
					if(update){
						...
						if(aRows.length  > num + 1 ){
							if( allRowUpdate == myUpdate.length )
								...
								FeeWpayRwOvertimeService.addList(rwOvertime, false);
	 ...
	 function compareDateBeforSave(){
	 	//document.forms['formInsert'].elements['refNo'].focus();
			...
			//alert('value in row :'+keySeq[i].value+' : '+flagPr[i].value+' : '+yearWork[i].value+' : '+periodWork[i].value+' : '+fiscalYear[i].value+' : '+startDate[i].value+' : '+endDate[i].value+' : '+totDay1[i].value+' : '+totDay15[i].value+' : '+totDay3[i].value+' : '+seqData[i].value+' : '+codeSeqNew[i].value+' : '+flagWork[i].value+' : '+orgCode[i+1].value);
				...
				var totone = parseInt(totDay1[i].value);
				var totonehalf = parseInt(totDay15[i].value);
				var tottriple = parseInt(totDay3[i].value);
				var spsone = parseInt(spsDay1[i].value);
				var spsonehalf = parseInt(spsDay15[i].value);
				var spstriple = parseInt(spsDay3[i].value);
								
				var sumtot = totone+totonehalf+tottriple+spsone+spsonehalf+spstriple;
				...
				if(stDate != null && stDate != '' && endDate != null && endDate != ''){
						...
						//}else if(endYear != fisYear){
										...
										document.forms['formEdit'].elements['confirmData'].disabled = true;
										break;
						}else if(endYear == yearEdit && (endMonth == monthWork) && (endDay > 15)){
										ss = false;
...
<body>
<form name="formEdit"  action="security.htm?reqCode=CTWPAYMT003" method="post">
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYUP003 ] บันทึกค่าล่วงเวลา/ค่าทำงานในวันหยุดพักผ่อน
		</td>
	</tr>
</table>
          ...
          <td align="center">
				...
				<table id="table"  bordercolor="#6699CC"  align="center" style="text-align: center;width:1550px;" border="1" cellpadding="0" cellspacing="0" >
					...
					<tbody id="dataTable">
							...
							<td id="totDay15" align="center" ><input type="text"  name="totDay15"  maxlength="2" align="center" style="text-align:right;width:100%;"  onchange="return checkHour(this);" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
							<td id="totDay1" align="center" ><input type="text"  name="totDay1"  maxlength="2" align="center" style="text-align:right;width:100%;"  onchange="return checkHour(this);" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
							<td id="totDay3" align="center" ><input type="text"  name="totDay3"  maxlength="2" align="center" style="text-align:right;width:100%;"  onchange="return checkHour(this);" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
			    ...
			    if(tab.rows.length>1){
					for(i=0; i<tab.rows.length; i++){
						...
						if (chk[i].checked){
								...
								FeeWpayRwOvertimeService.deleteRwOvertime(rwOvertime, {callback:onDeleteCallback});
								chDelete = true;
							}
