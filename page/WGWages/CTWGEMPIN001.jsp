...
<%
	...
	if( flagQY != null && flagQY.equals("MT") ){
		...
		empCodeFromEdit = request.getParameter("empCodeFromIns");
		empCodeToEdit = request.getParameter("empCodeToIns");
...
<html>
	<head>
		<title>เพิ่มข้อมูลประจำเดือนของลูกจ้าง (ข้อมูลหลัก)</title>
		<!-- Include -->
	           ...
	           var cbo = dojo.widget.byId("orgCode");
		    	...
		    	}else
		    	{
			    	...
			    	{	
						...
						var pDate = getDateFromFormat($("pDate").value,"dd/MM/yyyy");
						var ouCode =  '<%=ouCode%>';
						var userId = '<%=userId%>';
					    ...
					    FeeWgEmpWorkService.insertFeeWgEmployee(ouCode,empCode,userId, prefixName,firstName,lastName,codeSeqAct,pDate,empStatus,payStatus,marriedStatus, {callback:whenUpdateComplete});
			    	}else{
						alert("โปรดกรอกสังกัดปฏิบัติงานให้ถูกต้อง");
					}
		    	}
	...
	<body>
		<table width="100%">
			<tr>
				<td class="font-head">
					[ CTWGEMPIN001 ] เพิ่มข้อมูลบุคคลของลูกจ้างเหมา 	</td>
			</tr>
		</table>
		...
		<tr><td align="center">
			<TABLE border="0" align="center" width="920px" cellspacing="1">
				<TR>
					...
					<TD width="130px"><INPUT type="text" id="empCode" maxlength="13" style="width:130px;text-align: left;background-color:transparent;" /><INPUT type="hidden" id="hEmpCode"></TD>
				...
				<TR>
					...
					<TD><INPUT type="text" id="pDate" align="center" style="text-align:center;width:100%;" onchange="return validateDate(this);" ><INPUT type="hidden" id="hPdate" /></TD>
				    	...
				    	<TD class="font-field" style="text-align:right;width:100px">สภาพการจ้าง</TD>
							<TD width="100px">
								<SELECT id="empStatus" style="width:100px" ">
									...
									<OPTION value="A">เลิกจ้างมีโทษ</OPTION>
								</SELECT>
