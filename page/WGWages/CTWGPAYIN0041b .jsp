...
<html>
<head>
<title>Add feeWgMonthEmpWork</title>
<!-- Include -->
...
<table>
	<tr>
		<td class="font-head">[ CTWGPAYIN0041 ]  เพิ่มข้อมูลวันทำงานประจำเดือนของลูกจ้างเหมา</td>
	</tr>
</table>
...
<table width="400" border="0" align="center" cellspacing="1">
	...
	<tr>
		...
		<td >
			<select widgetId="empCode" dojoType="combobox" style="width:140;">
          	</select>
		</td>
		<td colspan="2">
...
<script>	
	...
	function findEmpName(){		
		FeeWgPrWorkDayService.findEmpName(DWRUtil.getValue("hidOuCode"), dojo.widget.byId("empCode").comboBoxSelectionValue.value, {callback:whenShowEmpName});
	}
	
	function whenShowEmpName(data){				
		DWRUtil.setValue("empName", data);
		FeeWgPrWorkDayService.findEmpByKey(DWRUtil.getValue("hidWorkYear"), DWRUtil.getValue("hidWorkMonth"), dojo.widget.byId("empCode").comboBoxSelectionValue.value, {callback:whenFindEmpName});
	}
	...
	function whenFindEmpName(data){	
		FeeWgPrWorkDayService.findStatusWork(DWRUtil.getValue("hidOuCode"),DWRUtil.getValue("hidWorkMonth") ,{callback:whenShowTable});
	}
	...
	function whenClickAdd(){
		...
		FeeWgPrWorkDayService.addPrWorkDay(ouCode, year, month, empCode, creby, arrDay, arrWorkCode, arrWorkSeq, {callback:whenAddComplete});
	}
	...
	function whenCallback(){
		var frm = document.forms[0];
		frm.action = "security.htm?reqCode=CTWGPAYMT0041";
		frm.submit();		
	}
