...
<script type="text/javascript">
	...
	function countData(){
		...
		FeeWpayRwVinai3Service.countData
		(
			'<%=userId%>',
			'<%=ouCode%>',
	...
	function whenShowDataTable()
	{
		...
		FeeWpayRwVinai3Service.findByCriteria
		(
			'<%=userId%>',
			'<%=ouCode%>',
			parseInt(DWRUtil.getValue("year")),
	    ...
	    if(tab.rows.length>1){
			for(i=0; i<tab.rows.length; i++){
				...
				if (chk[i].checked){
						...
						FeeWpayRwVinai3Service.deleteRwVinai3(emp, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
					}
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYMT014 ] รายการหักได้รับค่าจ้างไม่เต็มเดือน
		</td>
	</tr>
</table>
...
<table width="770" align="center"  cellpadding="2" cellspacing="0" >
	...
	<form name="insertForm" action="security.htm?reqCode=CTWPAYIN014" method="post">
	...
	<form name="editForm" action="security.htm?reqCode=CTWPAYUP014" method="post">
