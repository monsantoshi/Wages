...
<script type="text/javascript" src="dwr/interface/FeeWgRwVinai2Service.js"></script>
   ...
   /* function whenFetchEmployeeToCallback(data)
	...
	function init()
	{
		...
		function chkMainClose(){
				FeeWgPrPeriodLineService.findPeriodLine('<%=ouCode%>',$("year").value,$("period").value, '<%=userId%>', {callback:whenchkMainCloseCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร1');}});
		    }
	...
	{	
		...
		//alert(document.forms['searchForm'].elements['incDecCbo'].value);
			...
			//alert('query');
			// Query by Criteria
			FeeWgRwVinai2Service.findByCriteriaListApprove
			(
				'<%=userId%>',
				'<%=ouCode%>',
	...
	var feeWgRwVinai2 = {keySeq:null, ouCode:null, yearPr:null, periodPr:null,empCode:null,confirmFlag:null,
			...
			if(canSave){
				if(empNull){
					...
					for(var i=0; i<tab.rows.length; i++){
						update = false;
						row = tab.rows[i];
						feeWgRwVinai2.keySeq = parseInt(keySeq[i].value);
						feeWgRwVinai2.codeSeq = codeSeq[i].value;
						if(tab.rows.length == 1){
							feeWgRwVinai2.flagPr = flagPr.value;
						}else{
							feeWgRwVinai2.flagPr = flagPr[i].value;
						}
						if (empCode[i].value != '' && empCode[i].value != null){
							feeWgRwVinai2.empCode  = empCode[i].value;
						}
						else{
							feeWgRwVinai2.empCode  = null;
						}
						
						if (name[i].value != '' && name[i].value != null){
							feeWgRwVinai2.name  = name[i].value;
						}
						else{
							feeWgRwVinai2.name  = null;
						}
						if (yearWork[i].value != ''&& yearWork[i].value != null){
							feeWgRwVinai2.yearWork  = yearWork[i].value;
						}
						else{
							feeWgRwVinai2.yearWork  = null;
						}
						
						if (periodWork[i].value != '' && periodWork[i].value != null){
							feeWgRwVinai2.periodWork  = periodWork[i].value;
						}
						else{
							feeWgRwVinai2.periodWork  = null;
						}
						
						
						feeWgRwVinai2.approveFlag = approveF[i].value;
						feeWgRwVinai2.approveBy = '<%=userId%>';
						//feeWgRwVinai2.updBy = '<%=userId%>';
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWgRwVinai2Service.addListApprove(feeWgRwVinai2, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWgRwVinai2Service.addListApprove(feeWgRwVinai2, false);
					
								}else{
								if( allRowUpdate == myUpdate.length )
									FeeWgRwVinai2Service.addListApprove(feeWgRwVinai2, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
								else
									FeeWgRwVinai2Service.addListApprove(feeWgRwVinai2, false);
	...
	function insertNewData()
	{
		...
		if(empNull){
			...
			{
					insertStatus = true;
					feeWgRwVinai2.keySeq = null;
					feeWgRwVinai2.ouCode = '<%=ouCode%>';
					feeWgRwVinai2.yearPr = parseInt($("year").value);
					feeWgRwVinai2.periodPr = parseInt($("period").value);
					feeWgRwVinai2.empCode = oRows[i].cells["empCode"].childNodes[0].value;
					feeWgRwVinai2.name = oRows[i].cells["name"].childNodes[0].value;
					feeWgRwVinai2.codeSeq = oRows[i].cells["name"].childNodes[1].value;
					feeWgRwVinai2.flagPr = oRows[i].cells["flagPr"].childNodes[0].value;
					feeWgRwVinai2.yearWork = oRows[i].cells["yearWork"].childNodes[0].value;
					feeWgRwVinai2.periodWork = oRows[i].cells["periodWork"].childNodes[0].value;
					
					
					
					
					feeWgRwVinai2.approveFlag = oRows[i].cells["approveF"].childNodes[0].value;
					feeWgRwVinai2.approveBy = '<%=userId%>';
					feeWgRwVinai2.approveDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
					//feeWgRwVinai2.confirmFlag = 'N';
					//feeWgRwVinai2.updBy = '<%=userId%>';
					//feeWgRwVinai2.creBy = '<%=userId%>';
					//feeWgRwVinai2.creDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
	...
	function whenSelectEmpOptionInRow(object)
	 ...
	 function whenFetchEmployeeDetailInRowUpdate(empCode)
	 {
		...
		FeeWgPnEmployeeService.findByEmpCodeDetail('<%=userId%>',empCode, '<%=ouCode%>', parseInt(DWRUtil.getValue("year")), parseInt(DWRUtil.getValue("period")), {callback:whenFetchEmployeeDetailInRowUpdateCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร4');}});
	 }
	 ...
	 function whenFetchEmployeeDetailInRow(empCode)
	 {
		DWRUtil.useLoadingMessage("Loading ...");
		//alert(empCode+' : '+<%=ouCode%>+' : ' + DWRUtil.getValue("year")+' : '+ DWRUtil.getValue("period"))
		FeeWgPnEmployeeService.findByEmpCodeDetail('<%=userId%>',empCode, '<%=ouCode%>', parseInt(DWRUtil.getValue("year")), parseInt(DWRUtil.getValue("period")), {callback:whenFetchEmployeeDetailInRowCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร5');}});
	 }
	 ...
	 function whenFetchEmployeeDetailInRowCallback(data)
	 {
		...
		function countData()
		{
			...
			//if(document.forms['searchForm'].elements['incDecCbo'].value != null && document.forms['searchForm'].elements['incDecCbo'].value != ''){
				...
				// Query by Criteria
				FeeWgRwVinai2Service.countDataApprove
				(
					'<%=userId%>',
					'<%=ouCode%>',
					parseInt(DWRUtil.getValue("year")),
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYAPMT010] บันทึกรับรองรายการหักโดยได้รับค่าจ้างไม่เต็มเดือน (ลูกจ้างเหมา)
		</td>
	</tr>
</table>
				    ...
				    if(tab.rows.length>1){
						for(i=0; i<tab.rows.length; i++){
							...
							if (chk[i].checked){
									...
									//alert('BB'+rowDelete);
									feeWgRwVinai2.keySeq = parseInt(frm.elements["keySeq"][i].value);
									FeeWgRwVinai2Service.deleteRwVinai2(feeWgRwVinai2, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร7');}});
									chDelete = true;
								}
						...
						if(tab.rows.length==1){
							...
							if (chk[0].checked){
									...
									//alert('CC'+rowDelete);
									feeWgRwVinai2.keySeq = parseInt(frm.elements["keySeq"][0].value);
									FeeWgRwVinai2Service.deleteRwVinai2(feeWgRwVinai2, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร8');}});
									chDelete = true;
								}
