...
<html>
	<head>
		...
		<script type="text/javascript">
		    ...
		    function whenLoadPnEmployeeDeatilCallback(data){
		    	...
		    	if( data.firstName != null )
		    	{
			   	...
			   	$("codeSeq").value = checkNull(data.codeSeq,'STRING');
			    	...
			    	FeeWpayPrEmployeeService.findPrEmployee('<%=ouCode%>', $("year").value, $("period").value, $("empCode").value, '<%=userId%>',{callback:whenPrEmployeeLoadCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
			    }else
		    ...
		    function checkChild(obj){
		    	...
		    	if(obj.value > 5){
		    		alert('จำนวนบุตรรับค่าช่วยเหลือ ไม่เกิน 3 คน ยกเว้นแฝด');
		   ...
		   function checkBankId(obj){
		    	...
		    	if(obj.value.length !=10){
		    		alert('เลขที่บัญชีธนาคารไม่ครบ 10 หลัก');
	...
	<body>
		<table width="100%">
			<tr>
				<td class="font-head">
					[ CTWPAYUP001 ] แก้ไขข้อมูลหลักลูกจ้างในระบบค่าจ้าง (รวมทั้งลูกจ้างบรรจุใหม่)
				</td>
			</tr>
		</table>
