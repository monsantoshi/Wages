...
<script type="text/javascript">
	...
	function whenShowDataTable()
	{
		if(document.forms['searchForm'].elements['incDecCbo'].value != null && document.forms['searchForm'].elements['incDecCbo'].value != ''){
			...
			// Query by Criteria
			FeeWpayRwIncDecOtherService.findByCriteria
			(
				'<%=userId%>',
				'<%=ouCode%>',
	...
	function writeSelect(inname,emp,key)
	{
		...
		if(<c:out value='${ConfirmRwIncDecOther}' />){
			return "<div align='center' style='background-color:#CCCCCC;'><select name='"+inname+"' disabled='true' style='background-color:transparent;color:#000000'><option value='N' "+temp1+" >ปกติ</option>"+
												...
												"<option value='R' "+temp3+">รายการเรียกคืน</option></select></div>";
		}else {
				...
				if(empNull){
					...
					for(var i=0; i<tab.rows.length; i++){
						...
						if(tab.rows.length == 1){
						    ...
						    if (document.forms['searchForm'].elements['incDecCbo'].value=='10'){
						           if(flagPr.value=='N'){
					     			 rwIncDecOther.flagPr = 'A'
					     	      }else{
	...
	function insertNewData()
	{
		...
		if(empNull){
			...
			for(var i=oRows.length-1;i > num;i--)
			{
					...
					if (document.forms['searchForm'].elements['incDecCbo'].value=='10'){
					      if (oRows[i].cells["flagPr"].childNodes[0].value=='N'){
					      rwIncDecOther.flagPr = 'A'
					      }else{
	...
	function whenChengIncDec(){
		...
		var chkName;
		var num;
		alert("คีย์ค่าช่วยเหลือบุตร ได้เฉพาะรายการ ปรับปรุงรายการรับ หรือ เรียกคืน เท่านั้น");
		if(parseInt(DWRUtil.getValue("dataLength")) != null && parseInt(DWRUtil.getValue("dataLength")) > 0){
			num = 1 + parseInt(DWRUtil.getValue("dataLength"));
		}else{
			num = 1;
	...
	function checkValue(object){
		...
		if(amt != null && amt != '' && amt >= 0){
			FeeWpayPrIncomeDeductService.findMaxIncDecCode('<%=ouCode%>','2',strIncDec, {callback:whenCheckAmtTotalCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		}else{
	...
	function checkValueInRowUpdate(data){
		...
		if(totalAmt != null && totalAmt != '' && totalAmt >= 0){
				...
				FeeWpayPrIncomeDeductService.findMaxIncDecCode('<%=ouCode%>','2',strIncDec, {callback:whenCheckAmtTotalInRowUpdateCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		}else{
			alert('กรุณากรอกจำนวนเงิน');
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYMT008 ] บันทึกเงินรับอื่น ๆ
		</td>
	</tr>
</table>
<table width="770" border="0" align="center" cellspacing="1">
 	...
 	<tr>
		<td class="font-field" align="right">ประเภทเงินรับอื่น&nbsp;</td>
		<td align="left">
		...
		<table id="table" width="865"  border="1" bordercolor="#6699CC"   cellpadding="0" cellspacing="0">
			...
			<tr id="temprow" style="visibility:hidden;position:absolute">
				...
				<td id="flagPr" width="50" align="center">
						...
						<option value="R"  >รายการเรียกคืน</option>
				</select>
			</td>
			<td id="empCode" align="center"><input type="text" name="empCode" maxlength="6" style="width:100%" onchange="whenSelectEmpOptionInRow(this);"  onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"></td>
