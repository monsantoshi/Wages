...
<%
	...
	boolean isConfirmInCome = ((Boolean)session.getAttribute("isConfirmInCome")).booleanValue();
	
	System.out.println("isCloseTranClose : " + isCloseTranClose);
	System.out.println("isConfirm : " + isConfirm);
	System.out.println("isConfirmIncome : " + isConfirmInCome);
%>
<html>
<head>
...
var statusConfirm = 'N';
	function init()
	{
		...
		$("isConfirmInCome").value = '<%= isConfirmInCome %>';
		
		chkMainClose();
		if( $("isConfirmInCome").value == 'true' ){
			document.forms['searchForm'].elements['add'].disabled = true;
			alert("!!! มีต้นสังกัดยังไม่ยืนยันข้อมูล กรุณาไปหน้าตรวจสอบสถานะการยืนยันข้อมูล !!!");
			window.history.back()
		}
	...
	function whenShowDataTable()
	{
		...
		//alert("test");
		FeeWpayPrEmployeeTextService.updateApproveIncome
		(
			'<%=userId%>',
			'<%=ouCode%>',
		...
		if (statusConfirm == 'Y'){
		     alert("!!! กรุณารอจนกว่าระบบจะแจ้งว่าดำเนินการเรียบร้อยถึงออกจากหน้าจอนี้ !!!");
			FeeWpayRwConfirmDataService.insertRwConfirmDataApMt012
			(
				confirmDataVo,
	...
	function whenInsertRwConfirmDataHandler(data){
		...
		btn.disabled = true;
		alert("ระบบดำเนินการเรียบร้อยแล้ว");
	}
	...
	function whenShowDataTable2()
	{
		...
		FeeWpayPrEmployeeTextService.updateApproveDeduct
		(
			'<%=userId%>',
			'<%=ouCode%>',
			DWRUtil.getValue("year"),
	...
	function chkMainClose(){
			...
			function whenchkMainCloseCallback(data){
			   // alert(data.mainClose);
			  //  alert(data.tranClose);
				if(data.tranClose == 'Y'){
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYAPMT012 ] ยืนยันการบันทึกรับรอง รายการเงินได้/เงินหัก (ลูกจ้าง)
		</td>
	</tr>
</table>
					...
					<table width="350" border="0" align="center" cellspacing="1">
					  <tr>
					    ...
					    <td align="left">
					    	...
					    	<INPUT type="hidden" name="isConfirmInCome" />
					    </td>
					  </tr>
					</table>
