...
<script type="text/javascript">
	...
	{	
		myUpdate = new Array();
			...
			//alert('query');
			// Query by Criteria
			FeeWgRwOvertimeService.findByCriteriaListApprove
			(
					'<%=userId%>',
					orgFromCbo,
					orgToCbo,
	...
	var feeWgRwOvertime = {keySeq:null, ouCode:null, yearPr:null, periodPr:null,
			...
			if(canSave){
				if(empNull){
					...
					for(var i=0; i<tab.rows.length; i++){
							update = false;
						row = tab.rows[i];
						feeWgRwOvertime.keySeq = parseInt(keySeq[i].value);
						if(tab.rows.length == 1){
							feeWgRwOvertime.flagPr = flagPr.value;
						}else{
							feeWgRwOvertime.flagPr = flagPr[i].value;
						}
						...
						if (empCode[i].value != '' && empCode[i].value != null){
							feeWgRwOvertime.empCode  = empCode[i].value;
						}
						else{
							feeWgRwOvertime.empCode  = null;
						}
						
						if (name[i].value != '' && name[i].value != null){
							feeWgRwOvertime.name  = name[i].value;
						}
						else{
							feeWgRwOvertime.name  = null;
						}
						feeWgRwOvertime.codeSeq = codeSeq[i].value;
					
						if (workHour[i].value != ''&& workHour[i].value != null){
							feeWgRwOvertime.workHour  = workHour[i].value;
						}
						else{
							feeWgRwOvertime.workHour  = null;
						}
						
						if (refNo[i].value != ''&& refNo[i].value != null){
							feeWgRwOvertime.refNo  = refNo[i].value;
						}
						else{
							feeWgRwOvertime.refNo  = null;
						}
						
						if (amtDay1[i].value !=''){
							feeWgRwOvertime.amtDay1  = parseInt(amtDay1[i].value);
						}
						else{
							feeWgRwOvertime.amtDay1  = null;
						}
			
						if (amtDay15[i].value !=''){
							feeWgRwOvertime.amtDay15 = parseInt(amtDay15[i].value);
						}
						else{
							feeWgRwOvertime.amtDay15  = null;
						}
			
						if (amtDay3[i].value !=''){
							feeWgRwOvertime.amtDay3  = parseInt(amtDay3[i].value);
						}
						else{
							feeWgRwOvertime.amtDay3  = null;
						}
			
						if (totDay1[i].value !=''){
							feeWgRwOvertime.totDay1  = parseInt(totDay1[i].value);
						}
						else{
							feeWgRwOvertime.totDay1  = null;
						}
			
						if (totDay15[i].value !=''){
							feeWgRwOvertime.totDay15  = parseInt(totDay15[i].value);
						}
						else{
							feeWgRwOvertime.totDay15  = null;
						}
			
						if (totDay3[i].value !=''){
							feeWgRwOvertime.totDay3  = parseInt(totDay3[i].value);
						}
						else{
							feeWgRwOvertime.totDay3  = null;
						}
						
						if (seqData[i].value !='' && seqData[i].value != null){
							feeWgRwOvertime.seqData  = parseInt(seqData[i].value);
						}
						else{
							feeWgRwOvertime.seqData  = null;
						}
						
						
						//if (approveF[i].value != '' && approveF[i].value != null){
							feeWgRwOvertime.approveFlag  = approveF[i].value;
						//}
						//else{
						//	feeWpayRwOvertime.approveFlag  = null;
						//}
						
						feeWgRwOvertime.approveBy = '<%=userId%>';
						feeWgRwOvertime.approveDate = getDateFromFormat(sed,"dd/MM/yyyy");
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWgRwOvertimeService.addListApprove(feeWgRwOvertime, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWgRwOvertimeService.addListApprove(feeWgRwOvertime, false);
					
								}else{
								if( allRowUpdate == myUpdate.length )
									FeeWgRwOvertimeService.addListApprove(feeWgRwOvertime, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
								else
									FeeWgRwOvertimeService.addListApprove(feeWgRwOvertime, false);
	...
	function insertNewData()
	{
		...
		if(empNull){
			...
			{
					insertStatus = true;
					feeWgRwOvertime.keySeq = null;
					feeWgRwOvertime.ouCode = '<%=ouCode%>';
					feeWgRwOvertime.yearPr = parseInt($("year").value);
					feeWgRwOvertime.periodPr = parseInt($("period").value);
					feeWgRwOvertime.empCode = oRows[i].cells["empCode"].childNodes[0].value;
					feeWgRwOvertime.name = oRows[i].cells["name"].childNodes[0].value;
					//feeWpayRwOvertime.codeSeq = oRows[i].cells["orgCode"].childNodes[1].value;
					feeWgRwOvertime.flagPr = oRows[i].cells["flagPr"].childNodes[0].value;
					feeWgRwOvertime.otType = parseInt($("otType").value);
					feeWgRwOvertime.refNo =  oRows[i].cells["refNo"].childNodes[0].value;
					feeWgRwOvertime.workHour =  oRows[i].cells["workHour"].childNodes[0].value;
					feeWgRwOvertime.startDate= getDateFromFormat(oRows[i].cells["startDate"].childNodes[0].value,"dd/MM/yyyy");
					feeWgRwOvertime.endDate = getDateFromFormat(oRows[i].cells["endDate"].childNodes[0].value,"dd/MM/yyyy");
					if(oRows[i].cells["amtDay1"].childNodes[0].value != ''){
						feeWgRwOvertime.amtDay1 = oRows[i].cells["amtDay1"].childNodes[0].value;
					}else{
						feeWgRwOvertime.amtDay1 = null;
					}
					if(oRows[i].cells["amtDay15"].childNodes[0].value != ''){
						feeWgRwOvertime.amtDay15 = oRows[i].cells["amtDay15"].childNodes[0].value;
					}else{
						feeWgRwOvertime.amtDay15 = null;
					}	
					if(oRows[i].cells["amtDay3"].childNodes[0].value != ''){
						feeWgRwOvertime.amtDay3 = oRows[i].cells["amtDay3"].childNodes[0].value;
					}else{
						feeWgRwOvertime.amtDay3 = null;
					}	
					if(oRows[i].cells["totDay1"].childNodes[0].value != ''){
						feeWgRwOvertime.totDay1 = oRows[i].cells["totDay1"].childNodes[0].value;
					}else{
						feeWgRwOvertime.totDay1 = null;
					}	
					if(oRows[i].cells["totDay15"].childNodes[0].value != ''){
						feeWgRwOvertime.totDay15 = oRows[i].cells["totDay15"].childNodes[0].value;
					}else{
						feeWgRwOvertime.totDay15 = null;
					}	
					if(oRows[i].cells["totDay3"].childNodes[0].value != ''){
						feeWgRwOvertime.totDay3 = oRows[i].cells["totDay3"].childNodes[0].value;
					}else{
						feeWgRwOvertime.totDay3 = null;
					}	
										
					feeWgRwOvertime.seqData = oRows[i].cells["seqData"].childNodes[0].value;
					...
					feeWgRwOvertime.approveFlag = oRows[i].cells["approveF"].childNodes[0].value;
					feeWgRwOvertime.approveBy = '<%=userId%>';
					feeWgRwOvertime.approveDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
	...
	function whenSelectEmpOptionInRow(object)
	 ...
	 function whenFetchEmployeeDetailInRowUpdate(empCode)
	 {
		...
		FeeWgPnEmployeeService.findByEmpCodeDetail('<%=userId%>',empCode, '<%=ouCode%>', parseInt(DWRUtil.getValue("year")), parseInt(DWRUtil.getValue("period")), {callback:whenFetchEmployeeDetailInRowUpdateCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	 }
	 ...
	 function whenFetchEmployeeDetailInRow(empCode)
	 {
		DWRUtil.useLoadingMessage("Loading ...");
		//alert(empCode+' : '+<%=ouCode%>+' : ' + DWRUtil.getValue("year")+' : '+ DWRUtil.getValue("period"))
		FeeWgPnEmployeeService.findByEmpCodeDetail('<%=userId%>',empCode, '<%=ouCode%>', parseInt(DWRUtil.getValue("year")), parseInt(DWRUtil.getValue("period")), {callback:whenFetchEmployeeDetailInRowCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	 }
	 ...
	 function countData()
		{	
			myUpdate = new Array();
				...
				//alert('query');
				// Query by Criteria
				FeeWgRwOvertimeService.countDataApprove
				(
						'<%=userId%>',
						orgFromCbo,
						orgToCbo,
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYAPMT003 ] บันทึกรับรองค่าล่วงเวลา (ลูกจ้างเหมา)
		</td>
	</tr>
</table>
...
<table  width="800" border="0" cellspacing="0" cellpadding="0" align="center">
	<tr>
		...
		<table id="table" lign="center" style="text-align: center;width:1500px;"  border="1" bordercolor="#6699CC" cellpadding="0"  cellspacing="0">
			...
			<td id="empCode" align="center"><input type="text" maxlength="13" name="empCode" style="width:100%" onchange="whenSelectEmpOptionInRow(this);"  onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"></td>
				    ...
				    if(tab.rows.length>1){
						for(i=0; i<tab.rows.length; i++){
							...
							if (chk[i].checked){
									...
									//alert('BB'+rowDelete);
									feeWgRwOvertime.keySeq = parseInt(frm.elements["keySeq"][i].value);
									FeeWgOvertimeService.deleteRwOvertime(feeWgRwOvertime, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
									chDelete = true;
								}
						...
						if(tab.rows.length==1){
							...
							if (chk[0].checked){
									...
									//alert('CC'+rowDelete);
									feeWgRwOvertime.keySeq = parseInt(frm.elements["keySeq"][0].value);
									FeeWgRwOvertimeService.deleteRwOvertime(feeWgRwOvertime, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
									chDelete = true;
								}
