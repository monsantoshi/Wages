						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwPremiumService.addListApprove(rwPremium, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWpayRwPremiumService.addListApprove(rwPremium, false);
					
								}else{
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwPremiumService.addListApprove(rwPremium, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
								else
									FeeWpayRwPremiumService.addListApprove(rwPremium, false);
		...
		<table id="table" width="900"  border="1" bordercolor="#6699CC" cellpadding="0" cellspacing="0">
			<thead style="text-align: center">
				<tr CLASS="TABLEBULE2" style="height: 30px;">
					<th CLASS="TABLEBULE2" style="width:100px" rowspan="2" align="center">รับรอง</th>
