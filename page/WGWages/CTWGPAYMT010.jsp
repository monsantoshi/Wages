...
<script type="text/javascript">
	...
	function countData(){
		...
		FeeWgRwVinai2Service.countData
		(
			'<%=userId%>',
			'<%=ouCode%>',
	...
	function whenShowDataTable()
	{
		...
		FeeWgRwVinai2Service.findByCriteria
		(
			'<%=userId%>',
			'<%=ouCode%>',
			parseInt(DWRUtil.getValue("year")),
	    ...
	    if(tab.rows.length>1){
			for(i=0; i<tab.rows.length; i++){
				...
				if (chk[i].checked){
						...
						FeeWgRwVinai2Service.deleteRwVinai2(emp, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
					}
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYMT010 ] บันทึกรายการหักโดยได้รับค่าจ้างไม่เต็มเดือน จ้างเหมา
		</td>
	</tr>
</table>
...
<table width="770" align="center"  cellpadding="2" cellspacing="0" >
	...
	<form name="insertForm" action="security.htm?reqCode=CTWGPAYIN010" method="post">
	...
	<form name="editForm" action="security.htm?reqCode=CTWGPAYUP010" method="post">
