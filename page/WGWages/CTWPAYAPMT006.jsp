...
//dojo.addOnLoad(onLoadOrganization);
	...
	{	
		myUpdate = new Array();
			...
			//alert('query');
			// Query by Criteria
			FeeWpayRwHealthService.findByCriteriaListApprove
			(
				'<%=userId%>',
				'<%=ouCode%>',
	...
	{	
		...
		var	empToCbo = dojo.widget.byId("empToCbo");
			...
			//alert('query');
			// Query by Criteria
			FeeWpayRwHealthService.countDataApprove
			(
				'<%=userId%>',
				'<%=ouCode%>',
				parseInt(DWRUtil.getValue("year")),
	...
	function writeText(inname,emp,maxlength,textalign,key)
	{
		...
		if(<c:out value='${ConfirmRwHealth}' />){
			...
			return "<div align='center' ><input type='text' readonly='true'  name = '"+inname+"' onChange='addListUpdate("+key+");checkValueInRowUpdate("+key+");'  value='"+emp+"' align='center' maxlength='"+maxlength+"' style='text-align:"+textalign+";width:100%;' onkeyup='if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);'/></div>";
		}
	}
	...
	function writeTextID(inname,emp,maxlength,textalign,key)
	{
		...
		if(<c:out value='${ConfirmRwHealth}' />){																	
			...
			return "<div align='center' ><input type='text' readonly='true'   name = '"+inname+"' maxlength='6' onChange='whenSelectEmpOptionInRowUpdate("+key+");addListUpdate("+key+")' value='"+emp+"' align='center' maxlength='"+maxlength+"' style='text-align:"+textalign+";width:100%;'  onkeyup='if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);'/></div>";
		}
	}
	...
	var rwHealth = {keySeq:null, ouCode:null, yearPr:null, periodPr:null,empCode:null,
					yearWork:null,periodWork:null,codeSeq:null,totalHour:null,confirmFlag:null,
					seqData:null,flagPr:null,updBy:null,updDate:null,creBy:null,creDate:null,approveFlag:null,approveBy:null,approveDate:null};
			...
			if(canSave){
				if(empNull){
					...
					for(var i=0; i<tab.rows.length; i++){
						...
						rwHealth.approveBy = '<%=userId%>';
						rwHealth.approveFlag  = approveF[i].value;
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwHealthService.addListApprove(rwHealth, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWpayRwHealthService.addListApprove(rwHealth, false);
					
								}else{
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwHealthService.addListApprove(rwHealth, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
								else
									FeeWpayRwHealthService.addListApprove(rwHealth, false);
	...
	function insertNewData()
	{
		...
		if(empNull){
			...
			for(var i=oRows.length-1;i > num;i--)
			{
					...
					rwHealth.approveFlag = oRows[i].cells["approveF"].childNodes[0].value;
					rwHealth.approveBy = '<%=userId%>';
					rwHealth.approveDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
					if( i == (num + 1) )
							FeeWpayRwHealthService.addListApprove(rwHealth, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
						else
							FeeWpayRwHealthService.addListApprove(rwHealth, false);
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYAPMT006 ] บันทึกรับรองเงินบำรุงสุขภาพ
		</td>
	</tr>
</table>
		...
		<table id="table" width="750"  border="1" bordercolor="#6699CC" cellpadding="0" cellspacing="0">
			<thead style="text-align: center">
				<tr CLASS="TABLEBULE2" style="height: 30px;">
					...
					<th CLASS="TABLEBULE2" align="center">บันทึก</th>
				</tr>
