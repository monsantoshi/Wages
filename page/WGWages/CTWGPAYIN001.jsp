		...
		<script type="text/javascript">
		    ...
		    function whenEmpBlur()
		    {
		    	if( $("empCode").value != '' ){
			    	...
			    	FeeWgPnEmployeeService.findPnEmpDetailNotInSecue('<%=userId%>', '<%=ouCode%>', $("empCode").value, $("year").value, $("period").value,{callback:whenPnEmployeeDeatilCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		    	}
		    }
		    ...
		    function whenPnEmployeeDeatilCallback(data)
		    {
		    	if( data.firstName != null )
		    	{
			    	...
			    	FeeWgPrEmployeeService.findPrEmployee('<%=ouCode%>', $("year").value, $("period").value, $("empCode").value, '<%=userId%>',{callback:whenPrEmployeeBlurCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		    	}else{
		    ...
		    function whenCheckCanSaveCallback(data)
		    {
		    	...
		    	}else
		    	{
					...
					//if( !isEqualsValue($("flagPr").value, $("hFlagPr").value) )
						...
						FeeWgPrEmployeeTextService.insertPrEmployeeText(prEmpTextVo, {callback:whenInsertPrEmployeeText,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
					}else{
 ...
 alert('เลขบัตรประชาชนไม่ถูกต้อง');
	...
	<body>
		<table width="100%">
			<tr>
				<td class="font-head">
					[ CTWGPAYIN001 ] เพิ่มข้อมูลหลักลูกจ้างเหมาเข้างานค่าจ้าง
				</td>
			</tr>
		</table>
