...
<%
	...
	String periodEdit = request.getParameter("periodEdit");
	String sectionEdit = request.getParameter("sectionEdit");
...
<html>
	<head>
		...
		FeeWpayPrPeriodLineService.findPeriodInPeriodLine('<%=ouCode%>',DWRUtil.getValue("year"),'<%=userId%>',{callback:onLoadPeriodSectionCallback});
	}
	function onLoadPeriodSectionCallback(data){
		...
		dojo.widget.byId("periodTmp").comboBoxSelectionValue.value = DWRUtil.getValue("period");
			...
			function onChangePeriodSection(){
				FeeWpayPrPeriodLineService.findPeriodInPeriodLine('<%=ouCode%>',DWRUtil.getValue("year"),'<%=userId%>',{callback:onChangePeriodSectionCallback});
			}
	   ...
	   function whenQueryEmpBefore()
		    ...
		    function whenQueryEmp(){
				...
				FeeWpayPrEmployeeService.findByCriteriaRWQY001(
		     		'<%= ouCode %>', 
		    ...
		    function countData(){
				...
				FeeWpayPrEmployeeService.getCountByCriteriaRWQY001(
		     		'<%= ouCode %>', 
		     		DWRUtil.getValue("year"), 
		     		$("period").value, 
		     		'<%= userId %>',
			...
			function preEdit(empId){ 
				...
				//alert(' empID:'+DWRUtil.getValue("empCodeQuery")+ ' period:'+DWRUtil.getValue("periodEdit")+ ' section:'+DWRUtil.getValue("sectionEdit"));
				frm.submit();
			}
	...
	<body>
		...
		<table width="100%">
			<tr>
				<td class="font-head">
					[ CTWPAYQY001 ] สอบถามข้อมูลประจำเดือนของลูกจ้าง
				</td>
			</tr>
		</table>
				...
				<tr>
					<td colspan="2">
						...
						<!--	<tr>
								...
								<td align="left" width="150px">&nbsp;<input type="text" name="year"  value=""  style="width: 70px;text-align: "/></td>
							</tr>-->
						</table>
					</td>
			...
			<table width="660" align="center"  cellpadding="2" cellspacing="0" >
				<tr>
					<td align="right">
						...
						<input type="hidden" name="dataPerPage" value="15">
