...
<%
	...
	if( flagQY != null && flagQY.equals("MT") ){
	    ...
	    yearEdit = (String)request.getParameter("yearEdit");
	     periodEdit = (String)request.getParameter("periodEdit");
	      sectionEdit = (String)request.getParameter("sectionEdit");
	    ...
	    yearEdit = (String)request.getParameter("yearEdit2");
	    periodEdit = (String)request.getParameter("periodEdit2");
	    sectionEdit = (String)request.getParameter("sectionEdit2");
...
<html>
	<head>
		...
		<script type="text/javascript">
		    ...
		    function whenPnEmployeeDeatilCallback(data)
		    {
		    	...
		    	FeeWpayPrEmployeeService.findPrEmployee('<%=ouCode%>', '<%=yearEdit%>', '<%=periodEdit%>', '<%=empCode%>', '<%=userId%>',{callback:whenPrEmployeeBlurCallback});
		    }
	...
	<body>
		<table width="100%">
			<tr>
				<td class="font-head">
					[ CTWPAYQY001DT01 ] สอบถามข้อมูลประจำเดือนของลูกจ้าง (ข้อมูลหลัก)
				</td>
			</tr>
		</table>
		...
		<TABLE border="0" align="center" width="920px" cellspacing="1">
				<TR>
					...
					<TD width="130px"><INPUT type="text" id="empCode" readonly="readonly" style="width:130px;background-color:silver;" maxlength="6" onblur="whenEmpBlur();" onkeypress="clearScreen();" onkeydown="clearScreen();" /></TD>
	...
	<form name="editForm" action="security.htm?reqCode=CTWPAYQY001DT02" method="post">
