...
var statusConfirm = 'N';
	function init()
	{
		...
		$("isCloseTranClose").value = '<%= isCloseTranClose %>';
	...
	function whenShowDataTable2()
	{
		...
		(
			...
			DWRUtil.getValue("year"),
			DWRUtil.getValue("period"),
			{callback:whenListDataTable2Handler,errorHandler:function(message) { }}
		);
	}
	var allOrder = 0;
	var cellFuncs2 = [
	...
	function chkMasterClose(){
				...
				FeeWpayRwConfirmDataService.findByCriteriaFlagByUser('<%=userId%>',$("year").value,$("period").value , {callback:whenchkMasterCloseCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร6');}});
		    }
			
	function whenchkMasterCloseCallback(data){
	  
		if(data.flag2 == 0 ){
		alert('ไม่สามารถดำเนินการต่อได้ ท่านยังไม่ได้ปิดข้อมูลหลัก');
		window.history.back()
		}
	}
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYMT012 ] ยืนยันการบันทึกรายการเงินได้/เงินหัก
		</td>
	</tr>
</table>
