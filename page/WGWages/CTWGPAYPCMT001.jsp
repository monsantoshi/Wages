...
<html>
<head>
<title>คำนวณเงิน ค่าจ้างเหมา ทั้งหมดและปิดงวด</title>
<!-- Include -->
...
var statusConfirm = 'N';
	...
	function whenShowDataTable()
	{
		...
		DWRUtil.useLoadingMessage("Loading ...");
		//alert("test");
		FeeWgPrEmployeeTextService.submitIncomeTransClose
		(
			'<%=userId%>',
			'<%=ouCode%>',
		...
		if (statusConfirm == 'Y'){
			FeeWgRwConfirmDataService.insertRwConfirmDataPcMt001
			(
				confirmDataVo,
	...
	function whenShowDataTable2()
	{
		...
		FeeWgPrEmployeeTextService.submitDeductTransClose
		(
			'<%=userId%>',
			'<%=ouCode%>',
			DWRUtil.getValue("year"),
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYPCMT001 ] คำนวณค่าจ้างลูกจ้างเหมาทั้ง ปณท. และปิดงวด
		</td>
	</tr>
</table>
