...
<html>
		...
		<script type="text/javascript">
		    ...
		    function whenEmpBlur()
		    {
		    	if( $("empCode").value != '' ){
			    	...
			    	FeeWgPnEmployeeService.findByCriteriaInSecueEmpText('<%=userId%>', '<%=ouCode%>', $("empCode").value, $("year").value, $("period").value,{callback:whenLoadPnEmployeeDeatilCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		    	}
		    }
		    ...
		    function whenLoadPnEmployeeDeatilCallback(data){
		    	...
		    	if( data.firstName != null )
		    	{
			   	...
			   	$("codeSeq").value = checkNull(data.codeSeq,'STRING');
			    	...
			    	FeeWgPrEmployeeService.findPrEmployee('<%=ouCode%>', $("year").value, $("period").value, $("empCode").value, '<%=userId%>',{callback:whenPrEmployeeLoadCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
			    }else
		    ...
		    function whenCheckCanSaveCallback(data)
		    {
		    	...
		    	}else
		    	{
		    		...
		    		if( empName != '' && cbo.textInputNode.value != '' )
			    	{
						...
						FeeWgPrEmployeeTextService.insertPrEmployeeText(prEmpTextVo, {callback:whenInsertPrEmployeeText,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
					}else{
		    ...
		    function onDelete(){
		    	...
		    	if( answer ){
			    	...
			    	if( empName != 'space' ){
						...
						FeeWgPrEmployeeTextService.insertPrEmployeeText(prEmpTextVo, {callback:whenDeletePrEmployeeText,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
					}else{
						alert("โปรดกรอก เลขประจำตัว หรือ สังกัดปฏิบัติงานให้ถูกต้อง");
					}
				}
			...
			function validateCostLife(){
				...
				if( costLife.value != null && costLife.value != '' ){
					if( costLife.value == '800' || costLife.value == '1600' || costLife.value == '2400' || costLife.value == '3200' )
					{
						...
						alert('กรุณากรอกเงินยังชีพให้ถูกต้อง');
						costLife.focus();
					}
				}
			}
	...
	<body>
		<table width="100%">
			<tr>
				<td class="font-head">
					[ CTWGPAYUP001 ] แก้ไขข้อมูลหลักลูกจ้างเหมาในระบบค่าจ้าง (รวมทั้งลูกจ้างบรรจุใหม่)
				</td>
			</tr>
		</table>
		...
		<tr><td align="center">
			<TABLE border="0" align="center" width="920px" cellspacing="1">
				<TR>
					...
					<TD width="130px"><INPUT type="text" id="empCode" style="width:130px" maxlength="13" onblur="whenEmpBlur();" onkeypress="clearScreen();" onkeydown="clearScreen();" /></TD>
		...
		<FORM name="goMT" action="security.htm?reqCode=CTWGPAYMT001" method="post">
