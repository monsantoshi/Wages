...
<html>
<head>
<title>บันทึกจำนวนวันมาทำงานรายเดือน</title>
<!-- Include -->
...
<script type="text/javascript">
    ...
    function onLoadYearSectionCallback()
    {
		...
		$("confirm").value = "<c:out value='${ConfirmPrWorkDay}' /> "  ;//data.confirm;
		chkMainClose();
		...
		//PnEmployeeService.findPrEmpBySecurity('<%=userId%>','<%=ouCode%>',$("year").value,$("period").value, {callback:onLoadEmployeeCallback});
		 ...
		 if(<c:out value='${ConfirmPrWorkDay}' />){
					...
					document.forms['searchForm'].elements['generateData'].disabled = true;
...
//dojo.addOnLoad(onLoadOrganization);
	...
	function whenGenerate(){
		...
		FeeWpayPrWorkDayService.generate
			(
				'<%=userId%>',
				'<%=ouCode%>',
	...
	{	
		myUpdate = new Array();
			...
			//alert('query');
			// Query by Criteria
			FeeWpayPrWorkDayService.findByCriteriaList
			(
				'<%=userId%>',
				'<%=ouCode%>',
				parseInt(DWRUtil.getValue("year")),
	...
	var prWorkDay = {keySeq:null, ouCode:null, yearPr:null, periodPr:null,empCode:null,
					yearWork:null,periodWork:null,codeSeq:null,totalHour:null,confirmFlag:null,
					seqData:null,flagPr:null,updBy:null,updDate:null,creBy:null,creDate:null};
			...
			if(canSave){
				if(empNull){
					...
					for(var i=0; i<tab.rows.length; i++){
							update = false;
						row = tab.rows[i];
						prWorkDay.keySeq = parseInt(keySeq[i].value);
						
						
						
						prWorkDay.codeSeq = codeSeq[i].value;
						
						if(tab.rows.length == 1){
							prWorkDay.flagPr = flagPr.value;
						}else{
							prWorkDay.flagPr = flagPr[i].value;
						}
						
						prWorkDay.periodWork  = periodWork[i].value;
						
						if (yearWork[i].value != ''){
							prWorkDay.yearWork  = yearWork[i].value;
						}
						else{
							prWorkDay.yearWork  = null;
						}
						
						if (empCode[i].value != ''){
							prWorkDay.empCode  = empCode[i].value;
						}
						else{
							prWorkDay.empCode  = null;
						}
						
						if (name[i].value != ''){
							prWorkDay.name  = name[i].value;
						}
						else{
							prWorkDay.name  = null;
						}
						...
						if (totalHour[i].value !=''){
							prWorkDay.totalHour  = parseInt(totalHour[i].value);
						}
						else{
							prWorkDay.totalHour  = null;
						}
						
						
						if (seqData[i].value !=''){
							prWorkDay.seqData  = parseInt(seqData[i].value);
						}
						else{
							prWorkDay.seqData  = null;
						}
						
						prWorkDay.updBy = '<%=userId%>';
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWpayPrWorkDayService.addList(prWorkDay, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWpayPrWorkDayService.addList(prWorkDay, false);
					
								}else{
								if( allRowUpdate == myUpdate.length )
									FeeWpayPrWorkDayService.addList(prWorkDay, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
								else
									FeeWpayPrWorkDayService.addList(prWorkDay, false);
	...
	function insertNewData()
	{
		...
		if(empNull){
			...
			{
					insertStatus = true;
					prWorkDay.keySeq = null;
					prWorkDay.ouCode = '<%=ouCode%>';
					prWorkDay.yearPr = parseInt($("year").value);
					prWorkDay.periodPr = parseInt($("period").value);
					prWorkDay.empCode = oRows[i].cells["empCode"].childNodes[0].value;
					prWorkDay.name = oRows[i].cells["name"].childNodes[0].value;
					prWorkDay.codeSeq = oRows[i].cells["name"].childNodes[1].value;
					prWorkDay.flagPr = oRows[i].cells["flagPr"].childNodes[0].value;
					prWorkDay.yearWork = oRows[i].cells["yearWork"].childNodes[0].value;
					prWorkDay.periodWork = oRows[i].cells["periodWork"].childNodes[0].value;
					prWorkDay.totalHour = oRows[i].cells["totalHour"].childNodes[0].value;
					prWorkDay.seqData = oRows[i].cells["seqData"].childNodes[0].value;
					...
					prWorkDay.confirmFlag = 'N';
					prWorkDay.updBy = '<%=userId%>';
					prWorkDay.creBy = '<%=userId%>';
					prWorkDay.creDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
					//Service.addList(PrWorkDay, {callback:onInsertCallback});
	...
	function whenChengIncDec(){
		...
		if(str == '08' && !<c:out value='${ConfirmPrWorkDay}' />){
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYMT004 ] บันทึกจำนวนวันทำงานในเดือนนี้
		</td>
	</tr>
</table>
	  ...
	  <tr>
   	...
   	<tr>
	    ...
	    <td align="left"><SELECT  dojoType="ComboBox" widgetId="empToCbo" style="width:200"></SELECT></td>
  	    ...
  	    <td><input type="Button" value="Generate" id="generateData" name="generate" class=" button " onclick="whenGenerate();"  /></td>
  	</tr>
		...
		<table id="table" width="750"  border="1" bordercolor="#6699CC" cellpadding="0" cellspacing="0">
			<thead style="text-align: center">
				<tr CLASS="TABLEBULE2" style="height: 30px;">
					...
					<th CLASS="TABLEBULE2" style="width:100px" align="center"><center>จำนวนวันทำงาน</center></th>
			...
			<td id="totalHour" ><input type="text"  name="totalHour"  maxlength="4" style="text-align:right;width:100%;" onkeyup="if(this.value < 0) this.value = '';if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);"/></td>
				    ...
				    if(tab.rows.length>1){
						for(i=0; i<tab.rows.length; i++){
							...
							if (chk[i].checked){
									...
									//alert('BB'+rowDelete);
									prWorkDay.keySeq = parseInt(frm.elements["keySeq"][i].value);
									FeeWpayPrWorkDayService.deletePrWorkDay(prWorkDay, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
									chDelete = true;
								}
						...
						if(tab.rows.length==1){
							...
							if (chk[0].checked){
									...
									//alert('CC'+rowDelete);
									prWorkDay.keySeq = parseInt(frm.elements["keySeq"][0].value);
									FeeWpayPrWorkDayService.deletePrWorkDay(prWorkDay, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
									chDelete = true;
								}
