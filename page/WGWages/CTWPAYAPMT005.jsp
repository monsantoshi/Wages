...
<script type="text/javascript">
   ...
   /* function whenFetchEmployeeToCallback(data)
	...
	function writeSelect(inname,emp,key)
	{
		...
		return "<div align='center' ><select onchange='addListUpdate("+key+")' name='"+inname+"' disabled='true'><option value='N' "+temp1+" >ปกติ</option>"+
												"<option value='A' "+temp2+">ปรับปรุงรายการรับ</option>" +
												"<option value='R' "+temp3+">รายการรับเรียกคืน</option></select></div>";
	...
	function writeSelectMonth(inname,emp,key){
		...
		return "<div align='center' ><select onchange='addListUpdate("+key+")' name='"+inname+"' disabled='true'>" +
												"<option value='1' "+mm1+" >มกราคม</option>" +
													"<option value='2' "+mm2+">กุมภาพันธ์</option>" +
													"<option value='3' "+mm3+">มีนาคม</option>" +
													"<option value='4' "+mm4+">เมษายน</option>" +
	...
	function writeText(inname,emp,maxlength,textalign,key)
	{
		...
		if(<c:out value='${ConfirmRwPremiem}' />){
			...
			return "<div align='center' ><input type='text' name = '"+inname+"' readonly='true'  onChange='addListUpdate("+key+");'  value='"+emp+"' align='center' maxlength='"+maxlength+"' style='text-align:"+textalign+";width:100%;' onkeyup='if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);'/></div>";
		}
	}
	...
	function writeTextID(inname,emp,maxlength,textalign,key)
	{
		...
		if(<c:out value='${ConfirmRwPremiem}' />){																	
			...
			return "<div align='center' ><input type='text' name = '"+inname+"' maxlength='6' readonly='true'  onChange='whenSelectEmpOptionInRowUpdate("+key+");addListUpdate("+key+")' value='"+emp+"' align='center' maxlength='"+maxlength+"' style='text-align:"+textalign+";width:100%;'  onkeyup='if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);'/></div>";
		}
	}
	...
	function writeTextYearWork(inname,emp,maxlength,textalign,key)
	{
		...
		if(<c:out value='${ConfirmRwPremiem}' />){
			...
			return "<div align='center' ><input type='text' name = '"+inname+"' readonly='true'  onchange='addListUpdate("+key+")' value='"+emp+"' align='center' maxlength='"+maxlength+"' size='6' style='text-align:"+textalign+";' onkeyup='if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);'/></div>";
		}
	}
	...
	var feeWpayRwPremium = {keySeq:null, ouCode:null, yearPr:null, periodPr:null,empCode:null,yearWork:null,periodWork:null,codeSeq:null,morDay:null,aftDay:null,evnDay:null,morHour:null,aftHour:null,
					 evnHour:null,amtMor:null,amtAft:null,amtEvn:null,flagPr:null,confirmFlag:null,seqData:null,updBy:null,updDate:null,creBy:null,creDate:null,approveFlag:null,approveBy:null};
			...
			if(canSave){
				if(empNull){
					...
					for(var i=0; i<tab.rows.length; i++){
						update = false;
						row = tab.rows[i];
						feeWpayRwPremium.keySeq = parseInt(keySeq[i].value);
						feeWpayRwPremium.codeSeq = codeSeq[i].value;
						if(tab.rows.length == 1){
							feeWpayRwPremium.flagPr = flagPr.value;
						}else{
							feeWpayRwPremium.flagPr = flagPr[i].value;
						}
						if (empCode[i].value != '' && empCode[i].value != null){
							feeWpayRwPremium.empCode  = empCode[i].value;
						}
						else{
							feeWpayRwPremium.empCode  = null;
						}
						
						if (name[i].value != '' && name[i].value != null){
							feeWpayRwPremium.name  = name[i].value;
						}
						else{
							feeWpayRwPremium.name  = null;
						}
						if (yearWork[i].value != ''&& yearWork[i].value != null){
							feeWpayRwPremium.yearWork  = yearWork[i].value;
						}
						else{
							feeWpayRwPremium.yearWork  = null;
						}
						
						if (periodWork[i].value != '' && periodWork[i].value != null){
							feeWpayRwPremium.periodWork  = periodWork[i].value;
						}
						else{
							feeWpayRwPremium.periodWork  = null;
						}
						
						if (morDay[i].value !=''){
							feeWpayRwPremium.morDay  = parseInt(morDay[i].value);
						}
						else{
							feeWpayRwPremium.morDay  = null;
						}
			
						if (morHour[i].value !=''){
							feeWpayRwPremium.morHour = parseInt(morHour[i].value);
						}
						else{
							feeWpayRwPremium.morHour  = null;
						}
			
						if (aftDay[i].value !=''){
							feeWpayRwPremium.aftDay  = parseInt(aftDay[i].value);
						}
						else{
							feeWpayRwPremium.aftDay  = null;
						}
			
						if (aftHour[i].value !=''){
							feeWpayRwPremium.aftHour  = parseInt(aftHour[i].value);
						}
						else{
							feeWpayRwPremium.aftHour  = null;
						}
			
						if (evnDay[i].value !=''){
							feeWpayRwPremium.evnDay  = parseInt(evnDay[i].value);
						}
						else{
							feeWpayRwPremium.evnDay  = null;
						}
			
						if (evnHour[i].value !=''){
							feeWpayRwPremium.evnHour  = parseInt(evnHour[i].value);
						}
						else{
							feeWpayRwPremium.evnHour  = null;
						}
						
						
						if (seqData[i].value !='' && seqData[i].value != null){
							feeWpayRwPremium.seqData  = parseInt(seqData[i].value);
						}
						else{
							feeWpayRwPremium.seqData  = null;
						}
						feeWpayRwPremium.approveFlag = approveF[i].value;
						feeWpayRwPremium.approveBy = '<%=userId%>';
						//feeWpayRwPremium.updBy = '<%=userId%>';
						...
						if(update){
							...
							if(aRows.length  > num + 1 ){
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwPremiumService.addListApprove(feeWpayRwPremium, false, {callback:onInsertCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
								else
									FeeWpayRwPremiumService.addListApprove(feeWpayRwPremium, false);
					
								}else{
								if( allRowUpdate == myUpdate.length )
									FeeWpayRwPremiumService.addListApprove(feeWpayRwPremium, true, {callback:ClearData,errorHandler:function(message) { alert('ไม่สามารถบันทึกได้');}});
								else
									FeeWpayRwPremiumService.addListApprove(feeWpayRwPremium, false);
	...
	function insertNewData()
	{
		...
		if(empNull){
			...
			{
					insertStatus = true;
					feeWpayRwPremium.keySeq = null;
					feeWpayRwPremium.ouCode = '<%=ouCode%>';
					feeWpayRwPremium.yearPr = parseInt($("year").value);
					feeWpayRwPremium.periodPr = parseInt($("period").value);
					feeWpayRwPremium.empCode = oRows[i].cells["empCode"].childNodes[0].value;
					feeWpayRwPremium.name = oRows[i].cells["name"].childNodes[0].value;
					feeWpayRwPremium.codeSeq = oRows[i].cells["name"].childNodes[1].value;
					feeWpayRwPremium.flagPr = oRows[i].cells["flagPr"].childNodes[0].value;
					feeWpayRwPremium.yearWork = oRows[i].cells["yearWork"].childNodes[0].value;
					feeWpayRwPremium.periodWork = oRows[i].cells["periodWork"].childNodes[0].value;
					
					if(oRows[i].cells["morDay"].childNodes[0].value != ''){
						feeWpayRwPremium.morDay = oRows[i].cells["morDay"].childNodes[0].value;
					}else{
						feeWpayRwPremium.morDay = null;
					}
					if(oRows[i].cells["morHour"].childNodes[0].value != ''){
						feeWpayRwPremium.morHour = oRows[i].cells["morHour"].childNodes[0].value;
					}else{
						feeWpayRwPremium.morHour = null;
					}	
					...
					if(oRows[i].cells["aftHour"].childNodes[0].value != ''){
						feeWpayRwPremium.aftHour = oRows[i].cells["aftHour"].childNodes[0].value;
					}else{
						feeWpayRwPremium.aftHour = null;
					}	
					if(oRows[i].cells["evnDay"].childNodes[0].value != ''){
						feeWpayRwPremium.evnDay = oRows[i].cells["evnDay"].childNodes[0].value;
					}else{
						feeWpayRwPremium.evnDay = null;
					}	
					if(oRows[i].cells["evnHour"].childNodes[0].value != ''){
						feeWpayRwPremium.evnHour = oRows[i].cells["evnHour"].childNodes[0].value;
					}else{
						feeWpayRwPremium.evnHour = null;
					}	
										
					feeWpayRwPremium.seqData = oRows[i].cells["seqData"].childNodes[0].value;
					
					//alert('start : ' + oRows[i].cells["start"].childNodes[0].childNodes.length);
					
					
					
					feeWpayRwPremium.approveFlag = oRows[i].cells["approveF"].childNodes[0].value;
					feeWpayRwPremium.approveBy = '<%=userId%>';
					//feeWpayRwPremium.approveDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
					//feeWpayRwPremium.confirmFlag = 'N';
					//feeWpayRwPremium.updBy = '<%=userId%>';
					//feeWpayRwPremium.creBy = '<%=userId%>';
					//feeWpayRwPremium.creDate = getDateFromFormat(<%=date%>,"dd/MM/yyyy");
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWPAYAPMT005 ] บันทึกรับรองเงินเพิ่มพิเศษเป็นกะ
		</td>
	</tr>
</table>
...
<table  width="800" border="0" cellspacing="0" cellpadding="0" align="center">
	<tr>
		...
		<table id="table" width="950"  border="1" bordercolor="#6699CC" cellpadding="0" cellspacing="0">
			<thead style="text-align: center">
				<tr CLASS="TABLEBULE2" style="height: 30px;">
					<th CLASS="TABLEBULE2" rowspan="2">ประเภทรายการ</th>
					<th CLASS="TABLEBULE2" style="width:100px" rowspan="2">เลขประจำตัว</th>
					<th CLASS="TABLEBULE2" style="width:180px" rowspan="2">ชื่อ - นามสกุล</th>
					<th CLASS="TABLEBULE2" colspan="2" align="center" rowspan="2">ปี/งวดที่ทำงาน</th>
					<th CLASS="TABLEBULE2" style="width:120px" colspan="2" rowspan="1" align="center"><center>จำนวนกะเช้า <br><font size="1">(08.00 - 16.00)</font></center></th>
					<th CLASS="TABLEBULE2" style="width:120px" colspan="2" rowspan="1" align="center"><center>จำนวนกะบ่าย <br><font size="1">(16.00 - 24.00)</font></center></th>
					<th CLASS="TABLEBULE2" style="width:120px" colspan="2" rowspan="1" align="center"><center>จำนวนกะกลางคืน <br><font size="1">(24.00 - 08.00)</font></center></th>
					<th CLASS="TABLEBULE2" style="width:50px" rowspan="2" align="center">ลำดับที่</th>
					<th CLASS="TABLEBULE2" rowspan="2">บันทึก</th>
				</tr>
				<tr CLASS="TABLEBULE2"  >
					<th CLASS="TABLEBULE2" style="width:60px" align="center">วัน </th>
					<th CLASS="TABLEBULE2" style="width:60px" align="center">ชม.</th>
					<th CLASS="TABLEBULE2" style="width:60px" align="center">วัน</th>
					<th CLASS="TABLEBULE2" style="width:60px" align="center">ชม.</th>
					<th CLASS="TABLEBULE2" style="width:60px" align="center">วัน</th>
					<th CLASS="TABLEBULE2" style="width:60px" align="center">ชม.</th>
		...
		if(tab.rows.length > 0){
			if(idx > (3+tab.rows.length)){
				if(oRowsCheck[rrCheck].cells["empCode"].childNodes[0].value != null && oRowsCheck[rrCheck].cells["empCode"].childNodes[0].value != ''){
					...
					alert('กรุณากรอกเลขประจำตัวก่อนเพิ่มข้อมูลตัวถัดไป1');
			...
			if(idx > 3){
				if(oRowsCheck[rrCheck].cells["empCode"].childNodes[0].value != null && oRowsCheck[rrCheck].cells["empCode"].childNodes[0].value != ''){
					...
					alert('กรุณากรอกเลขประจำตัวก่อนเพิ่มข้อมูลตัวถัดไป2');
				    ...
				    if(tab.rows.length>1){
						for(i=0; i<tab.rows.length; i++){
							...
							if (chk[i].checked){
									...
									//alert('BB'+rowDelete);
									feeWpayRwPremium.keySeq = parseInt(frm.elements["keySeq"][i].value);
									FeeWpayRwPremiumService.deleteRwPremium(feeWpayRwPremium, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
									chDelete = true;
								}
						...
						if(tab.rows.length==1){
							...
							if (chk[0].checked){
									...
									//alert('CC'+rowDelete);
									feeWpayRwPremium.keySeq = parseInt(frm.elements["keySeq"][0].value);
									FeeWpayRwPremiumService.deleteRwPremium(feeWpayRwPremium, {callback:onDeleteCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
									chDelete = true;
								}
