...
<!--
	...
	function whenSelectEmpOptionInRow(object)
	 ...
	 {
		DWRUtil.useLoadingMessage("Loading ...");
		FeeWgPnEmployeeService.findByEmpCodeDetailNotSecurityCTRW03('<%=userIdInsert%>', empCode, '<%=ouCodeInsert%>', '<%=yearInsert%>', '<%=periodInsert%>', {callback:whenFetchEmployeeDetailInRowCallback,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
	...
	function backFormMain(){
	 	...
	 	//document.forms['formInsert'].elements['refNo'].focus();
	  			while (object.tagName !=  'TR')
					...
					if(stDate != null && stDate != '' && endDate != null && endDate != ''){
						...
						//}else 
						if(flagPr=='N' && fiscalYear == strYear && (monthWork == strMonth) && (strDay > 10)){
							alert("วันที่เริ่มต้น ห้ามเบิกเกินวันที่ 10 ของเดือน");
							document.forms['formInsert'].elements['insertData'].disabled = true;
							document.forms['formInsert'].elements['deleteData'].disabled = true;
							document.forms['formInsert'].elements['confirmData'].disabled = true;
							oRows[rowToCompare].cells["startDate"].childNodes[0].focus();
				 	...
				 	if(stDate != null && stDate != '' && (endDate == null || endDate == '')){
						...
						//	oRows[rowToCompare].cells["endDate"].childNodes[0].value = '';
						//}else 
						if(flagPr== 'N' && yearWork == strYear && fiscalYear == strYear && (monthWork == strMonth) && (strDay > 10)){
							alert("วันที่เริ่มต้น ห้ามเบิกเกินวันที่ 10 ของเดือน");
							document.forms['formInsert'].elements['insertData'].disabled = true;
							document.forms['formInsert'].elements['deleteData'].disabled = true;
							document.forms['formInsert'].elements['confirmData'].disabled = true;
	 ...
	 function compareEndDate(object){
		...
		if(stDate != null && stDate != '' && endDate != null && endDate != ''){
			...
			if(flagPr=='N' && endYear == yearInsert && endMonth > monthWork){
					...
					oRows[rowToCompare].cells["endDate"].childNodes[0].value = '';
					//oRows[rowToCompare].cells["startDate"].childNodes[0].value = '';
			}else if(endYear == yearInsert && (monthWork == endMonth) && (endDay > 10)){
					alert("วันที่สิ้นสุด ห้ามเบิกเกินวันที่ 10 ของเดือน");
					document.forms['formInsert'].elements['insertData'].disabled = true;
					document.forms['formInsert'].elements['deleteData'].disabled = true;
					document.forms['formInsert'].elements['confirmData'].disabled = true;
					oRows[rowToCompare].cells["endDate"].childNodes[0].focus();
	 ...
	 function ChkFiscal(object){
 	 ...
 	 function checkHour(object){
		...
		var totone = oRows[rowToCompare].cells["totDay1"].childNodes[0].value;
		var tot15 = oRows[rowToCompare].cells["totDay15"].childNodes[0].value;
		var tot3 = oRows[rowToCompare].cells["totDay3"].childNodes[0].value;
		var spsone = oRows[rowToCompare].cells["spsDay1"].childNodes[0].value;
		var sps15 = oRows[rowToCompare].cells["spsDay15"].childNodes[0].value;
		var sps3 = oRows[rowToCompare].cells["spsDay3"].childNodes[0].value;
		if((totone%1)!= 0 ){
...
<body>
<form name="formInsert"  action="security.htm?reqCode=CTWGPAYMT003" method="post">
...
<table width="100%">
	<tr>
		<td class="font-head">
			[ CTWGPAYIN003 ] บันทึกค่าล่วงเวลา  จ้างเหมา
		</td>
	</tr>
</table>
				...
				<table id="table"  bordercolor="#0f1011" align="center" style="text-align: center;width:2200px;" border="1" cellpadding="0" cellspacing="0" >
					...
					<tbody>			
						<tr id="temprow" style="visibility:hidden;position:absolute">
							...
							<td id="empCode" align="center"><input type="text" maxlength="13" name="empCode" style="width:100%" onblur="whenSelectEmpOptionInRow(this);" onkeydown="whenNewEmpNo();"  onkeyup="if(isNaN(Number(this.value))) this.value = this.value.substring(0,this.value.length - 1);whenCheck();"></td>
