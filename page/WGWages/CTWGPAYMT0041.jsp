...
<html>
<head>
<title>บันทึกจำนวนวันทำงานในแต่ละเดือนของลูกจ้างเหมา</title>
<!-- Include -->
...
<table>
	<tr>
		<td class="font-head">[ CTWGPAYMT0041 ]  บันทึกจำนวนวันทำงานในแต่ละเดือนของลูกจ้างเหมา</td>
	</tr>
</table>
...
dojo.addOnLoad(setWork);
	...
	function whenShowDataTable(){	
		...
		if (!isNumber(workYear) || workYear == ''){
			...
			var ouCode = DWRUtil.getValue("hidOuCode");
			FeeWgPrWorkDayService.listMonthEmpWork(ouCode, workYear, workMonth, areaCode, secCode, workCode, userId, maxRowPerPage, pageNow, {callback:whenListDataTableHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}});
		}
	}
	...
	function countData(){
		...
		var workCode = splitCombo(dojo.widget.byId("workCode").textInputNode.value);
		var userId = DWRUtil.getValue("userId");
		FeeWgPrWorkDayService.getCountTaMonthEmpWork(ouCode, workYear, workMonth, areaCode, secCode, workCode, userId, {callback:countDataHandler,errorHandler:function(message) { alert('เกิดข้อผิดพลาดเกี่ยวกับการติดต่อสื่อสาร');}})
	}
	...
	function whenCheckStausUpdate(data){
		if (data == 'Y'){
			alert("ข้อมูลของงวดและปีนี้ถูกปิดแล้วไม่สามารถแก้ไขข้อมูลได้");
		}else{
			var frm = document.forms[0];		
			frm.action = "security.htm?reqCode=CTWGPAYUP0041";		
	...
	function whenClickEditButton(btnVal){
		...
		var ouCode = DWRUtil.getValue("hidOuCode");
		empNo = btnVal.name;
		FeeWgPrWorkDayService.getStatusUpdateEmployee(ouCode, workYear, workMonth, empNo, userId, {callback:whenCheckStausUpdate});
	}
	function whenClickDel(){
		...
		var userId = frm.userId.value;
		var ouCode = DWRUtil.getValue("hidOuCode");
		FeeWgPrWorkDayService.getStatusBeforeAddEmployee(ouCode, workYear, workMonth, areaCode, secCode, workCode, userId, {callback:whenCheckStausDel});
	}
	
	function whenCheckStausDel(data){
		if (data == 'Y'){
			alert("ข้อมูลของงวดและปีนี้ถูกปิดแล้วไม่สามารถลบข้อมูลได้");
	...
	function onDelete(){
			...
			FeeWgPrWorkDayService.deleteTaMonthEmpWork(DWRUtil.getValue("hidOuCode"), year, month, arrEmpCode, userId, {callback:onDeleteCallback});
			DWREngine.endBatch();
	}
	...
	function validateForm(){
		...
		if (!isNumber(workYear) || workYear == ''){
			...
			frm.action = "security.htm?reqCode=CTWGPAYIN0041";
			frm.selectAll.checked = false;
			var comB1 = dojo.widget.byId("areaCode");
			var comB2 = dojo.widget.byId("secCode");
			var comB3 = dojo.widget.byId("workCode");
	...
	function whenCheckStaus(data){
	    //alert("CheckStatus");
		if (data == 'Y'){
			alert("ข้อมูลของงวดและปีนี้ถูกปิดแล้วไม่สามารถเพิ่มข้อมูลได้");
		}else if(data == 'NOT_FOUND'){
			alert("ไม่พบข้อมูล กรุณาเลือกข้อมูลใหม่อีกครั้ง");
		}else{
			validateForm();
	...
	function whenClickAdd(){
		...
		var userId = frm.userId.value;
		var ouCode = DWRUtil.getValue("hidOuCode");
		FeeWgPrWorkDayService.getStatusBeforeAddEmployee(ouCode, workYear, workMonth, areaCode, secCode, workCode, userId, {callback:whenCheckStaus});
	}
...
// for Report
	function whenShowReport(){
		...
		frm.action="wgpayPr0041Rpt.htm?reqCode=doPrintReport";
		frm.target = "_blank";
		frm.submit();
		frm.target = "_self";
	}
	...
	function whenShowDataTableAfterDelete(){
		...
		var pageNow = DWRUtil.getValue("page");
		var ouCode = DWRUtil.getValue("hidOuCode");
		FeeWgPrWorkDayService.listMonthEmpWork(ouCode, workYear, workMonth, areaCode, secCode, workCode, userId, maxRowPerPage, pageNow, {callback:whenListDataTableAfeterDeleteHandler});
	}
	...
	function countDataAfterDelete(){
		...
		var workCode = splitCombo(dojo.widget.byId("workCode").textInputNode.value);
		var userId = DWRUtil.getValue("userId");
		FeeWgPrWorkDayService.getCountTaMonthEmpWork(ouCode, workYear, workMonth, areaCode, secCode, workCode, userId, {callback:countDataAfterDeleteHandler})
	}
