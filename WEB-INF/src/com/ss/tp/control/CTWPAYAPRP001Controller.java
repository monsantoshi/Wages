 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTWPAYAPRP001Controller extends MultiActionController {
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		// }
		//
		List empList = this.getFeeWpayRwIncDecOtherService().incOutAmtReport(userId,
				ouCode, year, period, incDecCode, flag);
		List sheetList = this.getFeeWpayRwIncDecOtherService()
				.incOutAmtReportCountSheet(userId, ouCode, year, period,
						incDecCode, flag);
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTWPAYAPRP001.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTWPAYAPRP001.xls");
		...
		FeeWpayPrDailyAmtReportVO voSheet = null;
		String nameSheet = "";
		...
		if (countSheetName > 0) {// for Sheet Name
			for (int i = 0; i < countSheetName - 1; i++) {
				voSheet = (FeeWpayPrDailyAmtReportVO) sheetNameList.get(i + 1);
		...
		if (sheetList.size() > 0) {
			voSheet = (FeeWpayPrDailyAmtReportVO) sheetList.get(0);
		...
		sheet[0].addCell(new Label(0, 1,"โปรแกรม CTWPAYAPRP001                                                                                    รายงาน"+(tempInDecName.equals("")?incDecName.equals("")?"ประเภทรายหัก":incDecName:tempInDecName),HeadLeftFormat));
		...
		if (empList.size() > 0) {
			...
			for (Iterator itt = empList.iterator(); itt.hasNext();) {
				FeeWpayPrDailyAmtReportVO vo = (FeeWpayPrDailyAmtReportVO) itt.next();
				count = count + 1;
				if (!tempInDecCode.equals(vo.getIncDecCode())) {
					if (!tempInDecCode.equals(vo.getIncDecCode())) {
						...
						sheet[j].addCell(new Label(0,1,"โปรแกรม CTWPAYAPRP001                                                                         รายงาน"+tempInDecName,HeadLeftFormat));
				...
				if (empList.size() == count) {
					...
					sheet[j].addCell(new Label(
							2 + 2,
							row,
							vo.getTotAmt() == null ? String.valueOf(0.0)
									: String.valueOf(df.format(vo.getTotAmt())),
							normalRightFormat));
					flagTotal = "sum";
					if (vo.getTotAmt() != null) {
						sumTotalAmt += vo.getTotAmt().doubleValue();
