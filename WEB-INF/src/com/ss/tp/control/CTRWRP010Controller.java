...
import com.ss.tp.dto.VRwVinaiVO;
 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTRWRP010Controller extends MultiActionController {
	...
	private VRwVinaiService getVRwVinaiService() {
		return (VRwVinaiService) this.getApplicationContext().getBean(
				"vRwVinaiService");
	}
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		List empList = this.getVRwVinaiService().rwVinaiReport(userId, ouCode,
				year, period, flag);
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTRWRP010.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTRWRP010.xls");
		...
		if (empList.size() > 0) {
			...
			for (Iterator itt = empList.iterator(); itt.hasNext();) {
				VRwVinaiVO vo = (VRwVinaiVO) itt.next();
				count = count + 1;
