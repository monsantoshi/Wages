 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTPNRP001Controller extends MultiActionController {
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTTARP001.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTTARP001.xls");
		...
		List tempFontWeb = this.getTaMonthEmpWorkService().findStatusWork(ouCode);// ï¿½ï¿½ status workCode ï¿½ï¿½
		...
		if (tempFontWeb.size() > 0) {
			...
			for (int i = 0; i < tempFontWeb.size(); i++) {
				tamVo = (TaMonthEmpWorkVO) tempFontWeb.get(i);
		...
		String nameAreaF = this.getTaReportService().findAreaDesc(orgCodeFrom);
		String nameAreaT = this.getTaReportService().findAreaDesc(orgCodeTo);
		List secList = this.getTaReportService().findListSecTaReportNew(ouCode,
		...
		List taReplist = this.getTaReportService().findListTaReportNew(ouCode,
				userId, workYear, workYearFrom, workYearTo, monthFrom, monthTo,
				orgCodeFrom, orgCodeTo, empCodeFrom, empCodeTo, choice,
				statusWorkCode, dataStatusFrom, dataStatusTo, sumDonKeyFrom,
				sumDonKeyTo, workCodeFrom, workCodeTo);
