 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTWPAYAPRP201Controller extends MultiActionController {
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		String key = null;
		String keyAll = null;
		CTWPAYRP201VO value = new CTWPAYRP201VO();
		CTWPAYRP201VO valueAll = new CTWPAYRP201VO();
		...
		List ctwpayrp201 = this.getFeeWpayPrEmployeeService().findDetailPaySlip(ouCode,
				userId, year, period,approveFlag);
		...
		List rpList = ctwpayrp201;

		CTWPAYRP201VO vo = null;
		CTWPAYRP201VO vo2 = null;

		response.setContentType("application/vnd.ms-excel");
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTWPAYRP201.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTWPAYRP201.xls");
		...
		if (rpList.size() > 0) {
			try {
				...
				for (int k = 0; k < rpList.size(); k++) {
					vo = (CTWPAYRP201VO) rpList.get(k);
					if (k + 1 < rpList.size())
						vo2 = (CTWPAYRP201VO) rpList.get(k + 1);
					...
					if (!divCodeChk.equals(vo.getDivCode())
						...
						CTWPAYRP201VO voTotal = new CTWPAYRP201VO();
						BeanUtils.copyProperties(voTotal, vo);
						value = (CTWPAYRP201VO) hash.get(key);
						if (value != null) {
							// sum value
					...
					if (!divCodeChk.equals(vo.getDivCode())
						...
						CTWPAYRP201VO voTotalAll = new CTWPAYRP201VO();
						BeanUtils.copyProperties(voTotalAll, vo);
						valueAll = (CTWPAYRP201VO) hashAll.get(keyAll);
						if (valueAll != null) {
							// sum value
					...
					if (!divCodeChk.equals(vo.getDivCode())
						...
						if (i == 0) {
							...
							// Pay Slip";
							 String otDesc="โปรแกรม CTWPAYRP201";
                             String content = "รายงานยอดสรุปในการจัดทำ Pay Slip (ลูกจ้าง)";
						...
						//	vDouble = vo.getCntEmp().intValue() + vo.getEmpI().intValue() - vo.getEmpD().intValue();
							...
							sheet1.addCell(new Label(0, 4,"จำนวนลูกจ้างทั้งหมด  "+ String.valueOf(vDouble) + " คน       แก้ไข  " + vo.getEmpA()+" รายการ    เพิ่ม  "+vo.getEmpI()+" รายการ    ลบ  "+vo.getEmpD()+" รายการ",header));
							i++;
							sheet1.addCell(new Label(0, 5,"จำนวนคนที่ได้รับเบี้ยกันดาร  " + vo.getGundan()+" คน    จำนวนคนที่ได้รับค่าช่วยเหลือบุตร  "+vo.getCostChild()+" คน    จำนวนบุตรที่ได้รับค่าช่วยเหลือ  "+vo.getSumCostChild()+" คน",header));
							i++;
							...
							if (test > 7) {
								...
								sheet1.mergeCells(0, startRow + i, 12, startRow
										+ i);
								sheet1.addCell(new Label(0, startRow+i,"จำนวนลูกจ้างทั้งหมด  "+ String.valueOf(vDouble) + " คน       แก้ไข  " + vo.getEmpA()+" รายการ    เพิ่ม  "+vo.getEmpI()+" รายการ    ลบ  "+vo.getEmpD()+" รายการ",header));
								
								i++;
								sheet1.mergeCells(0, startRow + i, 12, startRow
										+ i);
								sheet1.addCell(new Label(0, startRow+i,"จำนวนคนที่ได้รับเบี้ยกันดาร  " + vo.getGundan()+" คน    จำนวนคนที่ได้รับค่าช่วยเหลือบุตร  "+vo.getCostChild()+" คน    จำนวนบุตรที่ได้รับค่าช่วยเหลือ  "+vo.getSumCostChild()+" คน",header));
								i++;
					...
					if (!vo.getIncDecCode().equals("00")) {
						...
						if (vo.getIncDecCode().equals("11")) {
							...
							i++;
							List rwOverTime = this.getFeeWpayPrEmployeeService()
									.findDetailPaySlipOverTime(ouCode, userId,
											year, period, vo.getDivCode(),
											vo.getAreaCode(), vo.getSecCode());
							FeeWpayRwOvertimeVO voOt = new FeeWpayRwOvertimeVO();
							String flagPrDesc = "";
							...
							for (int l = 0; l < rwOverTime.size(); l++) {
								voOt = (FeeWpayRwOvertimeVO) rwOverTime.get(l);
						...
						if (vo.getIncDecCode().equals("12")) {
							...
							List rwPremium = this.getFeeWpayPrEmployeeService()
									.findDetailPaySlipPremium(ouCode, userId,
											year, period, vo.getDivCode(),
											vo.getAreaCode(), vo.getSecCode());
							FeeWpayRwPremiumVO voPremium = new FeeWpayRwPremiumVO();
							String flagPrDesc = "";
							String flagPrDescTemp = "";
							String flagStatus = "";
							for (int l = 0; l < rwPremium.size(); l++) {
								voPremium = (FeeWpayRwPremiumVO) rwPremium.get(l);
						...
						if (vo.getIncDecCode().equals("13")) {
							...
							List rwHealth = this.getFeeWpayPrEmployeeService()
									.findDetailPaySlipHealth(ouCode, userId,
											year, period, vo.getDivCode(),
											vo.getAreaCode(), vo.getSecCode());
							FeeWpayRwHealthVO voHealth = new FeeWpayRwHealthVO();
							String flagPrDesc = "";
							String flagPrDescTemp = "";
							String flagStatus = "";
							for (int l = 0; l < rwHealth.size(); l++) {
								voHealth = (FeeWpayRwHealthVO) rwHealth.get(l);
						...
						if (i % 25 == 0) {
							...
							vDouble = vo.getCntEmp().intValue();
							sheet1.mergeCells(0, startRow + i, 12, startRow + i);
							sheet1.addCell(new Label(0, startRow+i,"จำนวนลูกจ้างทั้งหมด  "+ cntEmp + " คน       แก้ไข  " + empA +" รายการ    เพิ่ม  "+empI+" รายการ    ลบ  "+ empD +" รายการ",header));
				...
				for (Iterator it = new TreeSet(hash.keySet()).iterator(); it
					...
					if (!divCodeChk.equals(value.getDivCode())
						...
						if (i == 0) {
							...
							sheet1.addCell(new Label(0, 4,"จำนวนลูกจ้างทั้งหมด  "+ value.getCntEmp() + " คน       แก้ไข  " + value.getEmpA()+" รายการ    เพิ่ม  "+value.getEmpI()+" รายการ    ลบ  "+value.getEmpD()+" รายการ",header));
							sheet1.addCell(new Label(0, 5,"จำนวนคนที่ได้รับเบี้ยกันดาร  " + value.getGundan() +" คน    จำนวนคนที่ได้รับค่าช่วยเหลือบุตร  "+value.getCostChild()+" คน    จำนวนบุตรที่ได้รับค่าช่วยเหลือ  "+value.getSumCostChild()+" คน",header));
							...
							i++;
							sheet1.mergeCells(0,startRow+i, 12,startRow+i);
							sheet1.addCell(new Label(0, startRow+i,"จำนวนลูกจ้างทั้งหมด  "+ value.getCntEmp() + " คน       แก้ไข  " + value.getEmpA()+" รายการ    เพิ่ม  "+value.getEmpI()+" รายการ    ลบ  "+value.getEmpD()+" รายการ",header));
							i++;
							sheet1.mergeCells(0,startRow+i, 12,startRow+i);
							sheet1.addCell(new Label(0, startRow+i,"จำนวนคนที่ได้รับเบี้ยกันดาร  " + value.getGundan() +" คน    จำนวนคนที่ได้รับค่าช่วยเหลือบุตร  "+value.getCostChild()+" คน    จำนวนบุตรที่ได้รับค่าช่วยเหลือ  "+value.getSumCostChild()+" คน",header));
				...
				for (Iterator it = new TreeSet(hashAll.keySet()).iterator(); it
						.hasNext();) {
					key = (String) it.next();
					value = (CTWPAYRP201VO) hashAll.get(key);
					...
					if (value.getIncDecCode().equals("00")) {
						if (i == 0) {
							...
							String otDesc="โปรแกรม CTWPAYRP201                                                                               รายงานยอดสรุปในการจัดทำ Pay Slip (ลูกจ้าง)";
			...
			sheet1.addCell(new Label(0, 4,"จำนวนลูกจ้างทั้งหมด  0 คน       แก้ไข  0 รายการ    เพิ่ม  0 รายการ    ลบ  0 รายการ",header));
