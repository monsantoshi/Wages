 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTRWRP006Controller extends MultiActionController {
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		List empList = this.getRwIncDecOtherService().incDecOtherReport(userId,
				ouCode, year, period, incDecCode, flag);
		List sheetList = this.getRwIncDecOtherService()
				.incDecOtherReportCountSheet(userId, ouCode, year, period,
						incDecCode, flag);
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTRWRP006.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTRWRP006.xls");
		...
		if (countSheetName == 0) {
			sheet[0].setName("๏ฟฝ๏ฟฝ่พบ๏ฟฝ๏ฟฝ๏ฟฝ๏ฟฝ๏ฟฝ๏ฟฝ");
		} else {
		...
		sheet[0].addCell(new Label(0, 1,"โปรแกรม CTRWRP006                                                                       รายงาน"+(tempInDecName.equals("")?incDecName.equals("")?"ประเภทรายได้":incDecName:tempInDecName),HeadLeftFormat));
		...
		if (empList.size() > 0) {
			...
			for (Iterator itt = empList.iterator(); itt.hasNext();) {
				...
				if (!tempInDecCode.equals(vo.getIncDecCode())) {
					...
					sheet[j].addCell(new Label(0,1,"โปรแกรม CTRWRP006                                                  รายงาน"+tempInDecName,HeadLeftFormat));
			...
			sheet[0].addCell(new Label(0 + 1, 0, ouDesc, HeadFormat));
			sheet[0].addCell(new Label(0,1,"โปรแกรม CTRWRP006                                            รายงาน"+(tempInDecName.equals("")?incDecName.equals("")?"ประเภทรายรับ":incDecName:tempInDecName),HeadLeftFormat));
			sheet[0].addCell(new Label(0,2, "พิมพ์โดย  "+userInfo.getUserName(), HeadLeftFormat));
			sheet[0].addCell(new Label(3, 2,"ประจำงวด "+section+" พ.ศ. "+year+"                                                   วันที่พิมพ์ : "+sdfPrint.format(gd.getTime()),headRight));
