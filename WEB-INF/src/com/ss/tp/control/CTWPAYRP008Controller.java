 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTWPAYRP008Controller extends MultiActionController {
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		List empList = this.getFeeWpayRwIncDecOtherService().incDecOtherReport(userId,ouCode, year, period, incDecCode, flag);
		List sheetList = this.getFeeWpayRwIncDecOtherService().incDecOtherReportCountSheet(userId, ouCode, year, period,incDecCode, flag);
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTWPAYRP008.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTWPAYRP008.xls");
		...
		if (countSheetName > 0) {// for Sheet Name
			for (int i = 0; i < countSheetName - 1; i++) {
				...
				ww.copySheet("Sheet1", voSheet.getIncDecName(),ww.getNumberOfSheets());
		...
		sheet[0].addCell(new Label(0, 1,"โปรแกรม CTWPAYRP008                                                                       รายงาน"+(tempInDecName.equals("")?incDecName.equals("")?"ประเภทรายได้":incDecName:tempInDecName),HeadLeftFormat));
		...
		if (empList.size() > 0) {
			...
			for (Iterator itt = empList.iterator(); itt.hasNext();) {
				...
				if (!tempInDecCode.equals(vo.getIncDecCode())) {
					...
					sheet[j].addCell(new Label(0,1,"โปรแกรม CTWPAYRP008                                                  รายงาน"+tempInDecName,HeadLeftFormat));
				...
				if (!orgDesc.equals(vo.getOrgDesc())
					...
					sheet[j].setRowView(row, 320);
					// System.out.println("_____________________---------------____________:::
					// "+vo.getOrgDesc()+" ::: ROW ::: "+row);
			...
			sheet[0].addCell(new Label(0 + 1, 0, ouDesc, HeadFormat));
			sheet[0].addCell(new Label(0,1,"โปรแกรม CTWPAYRP008                                            รายงาน"+(tempInDecName.equals("")?incDecName.equals("")?"ประเภทรายรับ":incDecName:tempInDecName),HeadLeftFormat));
			sheet[0].addCell(new Label(0,2, "พิมพ์โดย  "+userInfo.getUserName(), HeadLeftFormat));
			sheet[0].addCell(new Label(3, 2,"ประจำงวด "+section+" พ.ศ. "+year+"                                                   วันที่พิมพ์ : "+sdfPrint.format(gd.getTime()),headRight));
