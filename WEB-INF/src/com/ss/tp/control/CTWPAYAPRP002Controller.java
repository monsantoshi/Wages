 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTWPAYAPRP002Controller extends MultiActionController {
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		// }
		//
		List empList = this.getFeeWpayRwIncDecOtherService().decOutAmtReport(userId,
				ouCode, year, period, incDecCode);
		List sheetList = this.getFeeWpayRwIncDecOtherService().decOutAmtReportCountSheet(userId, ouCode, year, period,
						incDecCode);
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTWPAYAPRP002.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTWPAYAPRP002.xls");
		...
		FeeWpayPrDailyOutAmtReportVO voSheet = null;
		String nameSheet = "";
		...
		if (countSheetName > 0) {// for Sheet Name
			for (int i = 0; i < countSheetName - 1; i++) {
				voSheet = (FeeWpayPrDailyOutAmtReportVO) sheetNameList.get(i + 1);
		...
		if (sheetList.size() > 0) {
			voSheet = (FeeWpayPrDailyOutAmtReportVO) sheetList.get(0);
		...
		CellFormat headRight = sheet[0].getWritableCell(0 + 1, 2).getCellFormat();
		...
		sheet[0].addCell(new Label(0, 1,"โปรแกรม CTWPAYAPRP002                                                                                    รายงาน"+(tempInDecName.equals("")?incDecName.equals("")?"ประเภทรายหัก":incDecName:tempInDecName),HeadLeftFormat));
		...
		if (empList.size() > 0) {
			...
			double sumOutAmt = 0.0;
			double sumAllOutAmt = 0.0;
			double sumNetAmt = 0.0;
			double sumAllNetAmt = 0.0;
			String flagTotal = "";
			for (Iterator itt = empList.iterator(); itt.hasNext();) {
				FeeWpayPrDailyOutAmtReportVO vo = (FeeWpayPrDailyOutAmtReportVO) itt.next();
				count = count + 1;
					...
					if (!tempInDecCode.equals(vo.getIncDecCode())) {
						if (!flagTotal.equals("")) {
							...
							sheet[j].addCell(new Label(3 + 2, row,
									convertFormatHour(new Double(sumOutAmt)),
									borderNumber2));
							sheet[j].addCell(new Label(4 + 2, row, 
									convertFormatHour(new Double(sumNetAmt)),
									borderNumber2));
							sheet[j].addCell(new Label(5 + 2, row, "",
									borderNumber2));
							sumAllTotalAmt = sumAllTotalAmt + sumTotalAmt;
							sumTotalAmt = 0.0;
							sumAllOutAmt = sumAllOutAmt + sumOutAmt;
							sumOutAmt = 0.0;
							sumAllNetAmt = sumAllNetAmt + sumNetAmt;
							sumNetAmt = 0.0;
							flagTotal = "";
							row++;
							...
							sheet[j].addCell(new Label(
									...
									borderNumber2));
							sheet[j].addCell(new Label(3 + 2, row, 
									convertFormatHour(new Double(sumAllOutAmt)),
									borderNumber2));
							sheet[j].addCell(new Label(4 + 2, row, 
									convertFormatHour(new Double(sumAllNetAmt)),
									borderNumber2));
							sheet[j].addCell(new Label(5 + 2, row, "",
									borderNumber2));
							sumAllTotalAmt = 0.0;
						...
						sheet[j].addCell(new Label(0,1,"โปรแกรม CTWPAYAPRP002                                                                         รายงาน"+tempInDecName,HeadLeftFormat));
				...
				if (!orgDesc.equals(vo.getOrgDesc())&& !"".equals(ChgNullToEmpty(vo.getOrgDesc().trim(), ""))) {
					if (!flagTotal.equals("")) {
						sheet[j].mergeCells(0, row, 1 + 2, row);
						sheet[j].addCell(new Label(0, row, "รวม", borderNumber2));
						sheet[j].addCell(new Label(2 + 2, row,
				...
				if (empList.size() == count) {
					...
					sheet[j].addCell(new Label(1 + 1, row, vo.getFullName(),
					...
					sheet[j].addCell(new Label(
							3 + 2,
							row,
							vo.getOutAmt() == null ? String.valueOf(0.0)
									: String.valueOf(df.format(vo.getOutAmt())),
							normalRightFormat));
					
					if (vo.getOutAmt() != null) {
						sumOutAmt += vo.getOutAmt().doubleValue();

					}
					sheet[j].addCell(new Label(
							4 + 2,
							row,
							vo.getNetAmt() == null ? String.valueOf(0.0)
									: String.valueOf(df.format(vo.getNetAmt())),
							normalRightFormat));
					
					if (vo.getNetAmt() != null) {
						sumNetAmt += vo.getNetAmt().doubleValue();

					}
				
				
					sheet[j].addCell(new Label(5 + 2, row, "",normalLeftFormat));
