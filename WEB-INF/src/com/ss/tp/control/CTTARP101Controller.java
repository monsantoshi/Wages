 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTTARP101Controller extends MultiActionController {
	 ...
	 * @return Returns the taReportService.
	 */
	public TaWgReportService getTaWgReportService() {

		return (TaWgReportService) this.getApplicationContext().getBean("taWgReportService");
	}
	...
	public TaWgMonthEmpWorkService getTaWgMonthEmpWorkService() {
		return (TaWgMonthEmpWorkService) this.getApplicationContext().getBean("taWgMonthEmpWorkService");
	}
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTTARP101.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTTARP101.xls");
		...
		List tempFontWeb = this.getTaWgMonthEmpWorkService().findStatusWork(ouCode);// ï¿½ï¿½ status workCode ï¿½ï¿½
		...
		if (tempFontWeb.size() > 0) {
			...
			for (int i = 0; i < tempFontWeb.size(); i++) {
				tamVo = (TaWgMonthEmpWorkVO) tempFontWeb.get(i);
		...
		String nameAreaF = this.getTaWgReportService().findAreaDesc(orgCodeFrom);
		String nameAreaT = this.getTaWgReportService().findAreaDesc(orgCodeTo);
		List secList = this.getTaWgReportService().findListSecTaWgReportNew(ouCode, userId, workYear,
						workYearFrom, workYearTo, monthFrom, monthTo,
						orgCodeFrom, orgCodeTo, empCodeFrom, empCodeTo, choice,
						statusWorkCode, dataStatusFrom, dataStatusTo, sumDonKeyFrom,
						sumDonKeyTo, workCodeFrom, workCodeTo);
		List taReplist = this.getTaWgReportService().findListTaWgReportNew(ouCode,
		...
		nameMonth.setAlignment(Alignment.CENTRE);
		...
		borderRight.setAlignment(Alignment.RIGHT);
		...
		dataBorder.setAlignment(Alignment.RIGHT);
		...
		// "ï¿½Ñ¹ï¿½ï¿½ï¿½ï¿½~ï¿½ : "+sdfPrint.format(gd.getTime()),headRight));
