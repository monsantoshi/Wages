...
import com.ss.tp.dto.WeEmployeeTextVO;
 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTRWPNT001Controller extends MultiActionController {

	/**
	 * @return Returns the taReportService.
	 */
	public WePnEmpTextReportService getWePnEmpTextReportService() {

		return (WePnEmpTextReportService) this.getApplicationContext().getBean(
				"wePnEmpTextReportService");
	}
	...
	public WePnEmployeeTextService getWePnEmployeeTextService() {
		return (WePnEmployeeTextService) this.getApplicationContext().getBean(
				"wePnEmployeeTextService");
	}
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTRWPNTRP001.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTRWPNTRP001.xls");
		...
		List secList = this.getWePnEmpTextReportService()
				.findListSecPntReportNew(ouCode, userId, orgCodeFrom,
						orgCodeTo, empCodeFrom, empCodeTo);
		List taReplist = this.getWePnEmpTextReportService()
		...
		WePnEmpTextReportVO vo = null;

		if (secList.size() > 0) {
			vo = (WePnEmpTextReportVO) secList.get(0);
		...
		for (int i = 0; i < numOfSheet - 1; i++) {
			tavo = (WePnEmpTextReportVO) nameSecList.get(i + 1);
		...
		try {
			if (taReplist.size() > 0) {
				for (Iterator itt = taReplist.iterator(); itt.hasNext();) {
					tavo = (WePnEmpTextReportVO) itt.next();
