 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTWPAYRP014bController extends MultiActionController {
	...
	private FeeWpayRwVinai3Service getFeeWpayRwVinai3Service() {
		return (FeeWpayRwVinai3Service) this.getApplicationContext().getBean("feeWpayRwVinai3Service");
	}
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		List empList = this.getFeeWpayRwVinai3Service().rwVinai3Report(userId, ouCode,
				year, period, flag);
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTWPAYRP014.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTWPAYRP014.xls");
		...
		if (flag.equals("N")){
			sheet1.addCell(new Label(0, 3,"รายการปกติ",HeadFormat));
		}
		else if(flag.equals("A")){
			sheet1.addCell(new Label(0, 3,"ปรับปรุงรายการหัก",HeadFormat));
		}else {
		...
		if (empList.size() > 0) {
			...
			for (Iterator itt = empList.iterator(); itt.hasNext();) {
				VFeeWpayRwVinai3VO vo = (VFeeWpayRwVinai3VO) itt.next();
				count = count + 1;
				...
				if (empList.size() == count) {
					...
					if (vo.getFlagPr().equals("N")){
						sheet1.addCell(new Label(5, row,"",normalRightLastFormat));
					}else if(vo.getFlagPr().equals("A")){
						sheet1.addCell(new Label(5, row,"ปรับปรุงรายการหัก",normalRightLastFormat));
					}
					countRow++;
					flagSum = "sum";
					...
					if (vo.getFlagPr().equals("N")){
							sheet1.addCell(new Label(5, row,"",normalRightFormat));
					}else if(vo.getFlagPr().equals("A")){
							sheet1.addCell(new Label(5, row,"ปรับปรุงรายการหัก",normalRightFormat));
					}
					countRow++;
					flagSum = "sum";
