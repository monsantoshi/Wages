 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CTRWRP017Controller extends MultiActionController {
	...
	public ModelAndView doPrintReport(HttpServletRequest request,
		...
		if (request.getParameter("incDecTmp") != null) {
			...
			incDecName = this.getPrIncomeDeductService().getIncDecName(ouCode,
					2, incDecCode);
		}
		...
		// }
		//
		List empList = this.getRwIncDecOtherService().decOutAmtReport(userId,
				ouCode, year, period, incDecCode);
		List sheetList = this.getRwIncDecOtherService()
				.decOutAmtReportCountSheet(userId, ouCode, year, period,
						incDecCode);
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=./page/report/CTRWRP017.xls");
		InputStream in = this.getServletContext().getResourceAsStream(
				"/page/report/CTRWRP017.xls");
		...
		PrDailyOutAmtReportVO voSheet = null;
		String nameSheet = "";
		...
		if (countSheetName > 0) {// for Sheet Name
			for (int i = 0; i < countSheetName - 1; i++) {
				voSheet = (PrDailyOutAmtReportVO) sheetNameList.get(i + 1);
		...
		if (sheetList.size() > 0) {
			voSheet = (PrDailyOutAmtReportVO) sheetList.get(0);
		...
		sheet[0].addCell(new Label(0, 1,"โปรแกรม CTRWRP017                                                                                    รายงาน"+(tempInDecName.equals("")?incDecName.equals("")?"ประเภทรายหัก":incDecName:tempInDecName),HeadLeftFormat));
		...
		if (empList.size() > 0) {
			...
			for (Iterator itt = empList.iterator(); itt.hasNext();) {
				PrDailyOutAmtReportVO vo = (PrDailyOutAmtReportVO) itt.next();
				count = count + 1;
				if (!tempInDecCode.equals(vo.getIncDecCode())) {
					if (!tempInDecCode.equals(vo.getIncDecCode())) {
						...
						sheet[0].addCell(new Label(0, 1,"โปรแกรม CTRWRP017                                                    รายงาน"+(tempInDecName.equals("")?incDecName.equals("")?"ประเภทรายหัก":incDecName:tempInDecName),HeadLeftFormat));
						sheet[0].addCell(new Label(0,2,"พิมพ์โดย  "+userInfo.getUserName()+"                      "+"ประจำงวด "+section+" พ.ศ. "+year+"     วันที่พิมพ์ : "+sdfPrint.format(gd.getTime()), HeadLeftFormat));
						//sheet[0].addCell(new Label(3, 2,"             ประจำงวด "+section+" พ.ศ. "+year+"                                   วันที่พิมพ์ : "+sdfPrint.format(gd.getTime()),headRight));
				...
				if (empList.size() == count) {
					...
					sheet[j].addCell(new Label(0 + 1, row, vo.getMemberNo(),
							normalCenterLastFormat));
					sheet[j].addCell(new Label(1 + 1, row, vo.getEmpCode(),
							normalCenterLastFormat));
					sheet[j].addCell(new Label(2 + 1, row, vo.getFullName(),
					...
					//sheet[j].addCell(new Label(4 + 2, row, flagDesc,
					//		normalLeftLastFormat));
