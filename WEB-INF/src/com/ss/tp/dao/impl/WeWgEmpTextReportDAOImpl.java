...
import com.ss.tp.dao.WeWgEmpTextReportDAO; // import com.ss.tp.dao.TaReportDAO;
...
import com.ss.tp.model.VWeWgEmpText;
 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class WeWgEmpTextReportDAOImpl extends HibernateDaoSupport implements
		WeWgEmpTextReportDAO, Serializable {
	...
	public List findListPntReport(String ouCode, String userId,
			...
			String empCodeTo) {
		VWeWgEmpText vtm;
		WeWgEmpTextReportVO trpVO;
		List list = new ArrayList();
		List listReport = new ArrayList();
		String sql = "from VWeWgEmpText as vt " + " where vt.codeSeq in( "
		...
		try {
			...
			for (Iterator itt = listReport.iterator(); itt.hasNext();) {
				vtm = (VWeWgEmpText) itt.next();
				trpVO = new WeWgEmpTextReportVO();
				BeanUtils.copyProperties(trpVO, vtm.getVweWgEmpTextPK());
	...
	public List findListPntReportNew(String ouCode, String userId,
		...
		try {
			...
			sql = " select a.ou_code,a.emp_code,a.Full_Name,a.preN,a.eng_name,a.eng_lastname,"
					...
					+ "\n from "
					+ "\n ( "
					+ "\n	 	select t.ou_code,t.emp_code,decode(t.prename,'1','Miss','2','Mrs.','3','Mr.') as preN,t.eng_name,t.eng_lastname, "
					+ "\n			(pr.PREFIX_NAME || ' ' ||pn.FIRST_NAME || ' ' || pn.LAST_NAME) as Full_Name, "
					+ "\n			pn.code_seq code_seq,org.org_code,org.org_desc, "
					...
					+ "\n	from WE_WG_EMPLOYEE_TEXT t,WG_EMPLOYEE pn, DB_PRE_SUFF pr,PN_ORGANIZATION org "
					...
					+ "\n		 "
					+ "\n		and 	   pn.code_seq  in ( select   vpn.CODE_SEQ "
			...
			sql += "\n ) a   "
					+ "\n   group by  a.ou_code,a.emp_code, a.full_name,a.preN,a.eng_name,a.eng_lastname,a.code_seq,a.org_code,a.org_desc, "
					...
					+ "\n order by a.div_code,nvl(a.area_seq,0),nvl(a.sec_seq,0),nvl(a.work_seq,0),a.emp_code ";
