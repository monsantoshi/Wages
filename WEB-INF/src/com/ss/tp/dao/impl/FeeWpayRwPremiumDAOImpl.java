...
public class FeeWpayRwPremiumDAOImpl extends HibernateDaoSupport implements
		FeeWpayRwPremiumDAO, Serializable {
	...
	public FeeWpayRwPremium loadRwPremium(FeeWpayRwPremiumVO rpVo) {
		FeeWpayRwPremium rwp = new FeeWpayRwPremium();
		try {
			Criteria c = this.getSessionFactory().openSession().createCriteria(FeeWpayRwPremium.class);
			c.add(Restrictions.idEq(rpVo.getKeySeq()));
	        rwp = (FeeWpayRwPremium) c.list().get(0);
	...
	public void updateRwPremium(FeeWpayRwPremiumVO rwpremiumvo) throws Exception {
		...
		try {
			...
			rw.setUpdDate(new Date());
			//rw.setApproveFlag(rwpremiumvo.getApproveFlag());
	...
	public void insertAndUpdateRwPremiums() throws Exception {
		try {
			for (int i = 0; i < this.rwList.size(); i++) {
				FeeWpayRwPremiumVO rwpremiumvo = (FeeWpayRwPremiumVO) this.rwList.get(i);
	...
	public void insertRwPremiums(List rwpremiumvolist) throws Exception {
		...
		try {
			for (int i = 0; i < rwpremiumvolist.size(); i++) {
				FeeWpayRwPremiumVO rwpremiumvo = (FeeWpayRwPremiumVO) rwpremiumvolist.get(i);
	...
	public List findByCriteria(String userId, String evaOuCode, Long evaYear,
		...
		hql.append(" from FeeWpayRwPremium rwPre , WgEmployee pnEmp , PnOrganization pnOrg, VPnOrganizationSecurity v ");
	...
	public FeeWpayPayRollEmployeeVO findByEmpCodeDetail(String empCode, String ouCode,
		...
		hql.append(" from FeeWpayRwPremium rwPre, ");
	...
	public List findByEmpCodeList(String userId, String ouCode, Long yearPr,
		...
		hql.append(" from FeeWpayRwPremium rw ");
		...
		if (empList != null && empList.size() > 0) {
			for (int i = 0; i < empList.size(); i++) {
				FeeWpayRwPremiumVO rw = new FeeWpayRwPremiumVO();
	...
	public List rwPremiumReportByOrg(String userId, String evaOuCode,
		...
		for (Iterator it = empList.iterator(); it.hasNext();) {
			...
			FeeRwPremiumReportByOrgVO ret = new FeeRwPremiumReportByOrgVO();
			ret.setEmpCode(empCode);
	...
	public List rwPremiumReportByOrg(String userId, String evaOuCode,
		...
		if (evaAppFlag != null && !evaAppFlag.equals(""))
			criteria.append(" and rwPre.approveFlag = '" + evaAppFlag + "' ");
	...
	public boolean isConfirmFlag(String ouCode, String year, String period,
		...
		sql.append(" from FeeWpayRwPremium p, VFeeWpPrEmployeeSecutiry e ");
...

	{
		...
		hql.append(" from FeeWpayRwPremium rwInc , WgEmployee pnEmp , PnOrganization pnOrg, VPnOrganizationSecurity v ");
		...
		try {
			for (int i = 0; i < this.rwList.size(); i++) {
				...
				if ((rwpremiumvo.getKeySeq() != new Long(0))
						&& rwpremiumvo.getKeySeq() != null
						&& !rwpremiumvo.getKeySeq().equals("")) {
					this.updateApTableApprove(rwpremiumvo);
				} else {
					this.insertRwPremium(rwpremiumvo);
				}
			}
	...
	//@Override
	public void updateApTableApprove(FeeWpayRwPremiumVO rwpremiumvo)throws Exception {
		FeeWpayRwPremium rw = new FeeWpayRwPremium();
		try {
			BeanUtils.copyProperties(rw, rwpremiumvo);
			rw = this.loadRwPremium(rwpremiumvo);
			rw.setApproveFlag(rwpremiumvo.getApproveFlag());
			rw.setApproveBy(rwpremiumvo.getApproveBy());
