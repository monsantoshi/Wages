 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class PeEmpScoreReportDAOImpl extends HibernateDaoSupport implements
		PeEmpScoreReportDAO, Serializable {
	...
	public List findPeEmpScoreReport(String ouCode, String userId,
			Integer evaYear, Integer evaTime, String areaCode, String secCode,
			String workCode) {
		PeEmpScoreReportVO peEmpScoreVO;
		List list = new ArrayList();
		List listDetail = new ArrayList();
		StringBuffer sql = new StringBuffer(0);
		...
		sql.append(" FROM ");
		sql.append(" 	PeEmployeeScore c, PeEmployeeScoreDetail d ,PnEmpDataStatus ed,PnOrganization o");
		sql.append(" WHERE  ");
		...
		try {
			...
			for (int n = 0; n < listDetail.size(); n++) {
				...
				peEmpScoreVO = new PeEmpScoreReportVO();
				peEmpScoreVO.setEvaYear((Long) ob[0]);
				peEmpScoreVO.setEvaTime((Long) ob[1]);
				peEmpScoreVO.setEmpCode((String) ob[2]);
				peEmpScoreVO.setEname((String) ob[3]);
				peEmpScoreVO.setAreaCode((String) ob[4]);
				peEmpScoreVO.setAreaDesc((String) ob[5]);
				peEmpScoreVO.setDivDesc((String) ob[6]);
				peEmpScoreVO.setSecCode((String) ob[7]);
				peEmpScoreVO.setSecDesc((String) ob[8]);
				peEmpScoreVO.setWorkCode((String) ob[9]);
				peEmpScoreVO.setWorkDesc((String) ob[10]);

				if (ob[11] != null) {
					pk1.setEmpCode((String) ob[11]);
					emp = (PnEmployee) getSession().load(PnEmployee.class, pk1);
					peEmpScoreVO.setEvaEmp1(emp.getRefDbPreSuff()
							.getPrefixName()
							+ " "
							+ emp.getFirstName()
							+ " "
				...
				if (ob[12] != null) {
					...
					peEmpScoreVO.setEvaEmp2(emp.getRefDbPreSuff()
							.getPrefixName()
							+ " "
							+ emp.getFirstName()
							+ " "
				...
				if (ob[13] != null) {
					...
					peEmpScoreVO.setEvaEmp3(emp.getRefDbPreSuff()
							.getPrefixName()
							+ " "
							+ emp.getFirstName()
							+ " "
				...
				peEmpScoreVO.setEvaTotal((Double) ob[15]);
				peEmpScoreVO.setFormCode((String) ob[16]);
				peEmpScoreVO.setFormDesc((String) ob[17]);
				...
				if (titleCodeList.size() > 0) {
					peEmpScoreVO.setTitleCodeGroup(titleCodeList);
				}

				if (scoreList.size() > 0) {
					peEmpScoreVO.setScoreGroup(scoreList);
				}

				peEmpScoreVO.setTitleDesc((String) ob[19]);

				list.add(peEmpScoreVO);
