 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class TaWgReportDAOImpl extends HibernateDaoSupport implements
		TaWgReportDAO, Serializable {
	...
	/**
	 * method ï¿½ï¿½ï¿½ï¿½Ñº Query ï¿½ï¿½ï¿½ï¿½ï¿½Å¨Ò¡ Table V_TA_WG_MONTH_EMP_WORK
	 * 
	 * @param workYear
	 * @param workMonth
	 * @param orgCodeSeqTo
	...
	public List findListTaReport(String ouCode, String userId,
		...
		String sql = "from VTaWgMonthEmpWork as vt " + " where vt.codeSeq in( "
		...
		if (workYear != null && workYear.intValue() != 0) {
			sql = sql + " and vt.vtaWgMonthEmpWorkPK.workYear  = " + workYear;
		...
		sql += " order by   nvl(vt.orgWorkCode,nvl(vt.secCode,nvl(vt.divCode,''))),vt.areaCode,"
				+ " vt.orgWorkCode, vt.vtaWgMonthEmpWorkPK.empCode,vt.vtaWgMonthEmpWorkPK.workCode ";
		...
		try {
			...
			for (Iterator itt = listReport.iterator(); itt.hasNext();) {
				...
				BeanUtils.copyProperties(trpVO, vtm.getVtaWgMonthEmpWorkPK());
		...
		try {
			...
			if ((sumDonKeyFrom.intValue() != 0 || sumDonKeyTo.intValue() != 0)
				...
				if (sumDonKeyFrom.intValue() != 0
					...
					sql += "\n 						and    ( work_code = (select ill_code from  ta_wg_status_control where ou_code = '"
							+ ouCode
							+ "') "
							+ "\n 							or work_code = (select bus_code from  ta_wg_status_control where ou_code = '"
							+ ouCode
							+ "') ) "
							+ "\n 							group by emp_code "
							+ " \n							having sum(total_days) between '"
